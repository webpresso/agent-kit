import type { ValidatorDeps } from '#cli/interfaces'
import type { DocType, ValidationError, ValidationResult } from '#types'
...
import { getNonCanonicalPlanningPathViolation } from '#path-contract'
import { detectDocType, getConfig, getSchema, normalizeDocType } from '#schemas'
import { validateCommandSafety } from '#validators/command-safety'
import { validateContextLimits } from '#validators/context-limits'
import { validateDeprecatedCommands } from '#validators/deprecated-commands'
import { validateFilename } from '#validators/filename'
...
export function createValidateCommand(): ValidateCommand {
  return new ValidateCommand({
    ...
    glob: (patterns: string[], opts: unknown) => glob(patterns, opts as Record<string, unknown>),
  })
}
