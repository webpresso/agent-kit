...
describe('@repo/workflow-skills provenance', () => {
  it('records upstream source, license, imported skills, and excluded payloads', () => {
    ...
    const notice = readFileSync(path.join(root, 'NOTICE.workflow-skills.md'), 'utf8')
    expect(notice).toContain('MIT-licensed upstream workflow project')
    expect(notice).toContain(`https://github.com/garrytan/${upstreamProject}`)
  })
})
