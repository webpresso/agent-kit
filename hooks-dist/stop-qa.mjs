...
// ../agent-kit/node_modules/.pnpm/glob@13.0.6/node_modules/glob/dist/esm/index.min.js
...
// ../agent-kit/src/hooks/stop/qa-changed-files.ts
import { execSync as execSync2 } from "node:child_process";
import { realpathSync as realpathSync2 } from "node:fs";
import { basename, dirname, extname as extname2, join } from "node:path";
...
// ../agent-kit/src/hooks/stop/qa-changed-files.ts
var TYPECHECKABLE_EXTENSIONS = /* @__PURE__ */ new Set([".ts", ".tsx"]);
function getChangedFiles(projectDir) {
  const unstaged = execSync2("git diff --name-only", { cwd: projectDir, encoding: "utf-8" }).trim();
  const staged = execSync2("git diff --cached --name-only", { cwd: projectDir, encoding: "utf-8" }).trim();
  const all = /* @__PURE__ */ new Set();
  for (const line of unstaged.split("\n")) if (line) all.add(line);
  for (const line of staged.split("\n")) if (line) all.add(line);
  return [...all];
}
function filterQaFiles(files) {
  return files.filter((f) => isLintableFile(f) && !isSkippedPath(f));
}
function getTypecheckFiles(files) {
  return files.filter((f) => TYPECHECKABLE_EXTENSIONS.has(extname2(f)));
}
function findTestFiles(sourceFile, projectDir) {
  const ext = extname2(sourceFile);
  ...
  return ts(pattern, { cwd: projectDir });
}
function discoverTestFiles(changedFiles, projectDir) {
...
function buildTypecheckCommand(files) {
  if (files.length === 0) return null;
  return `just typecheck ${files.map((f) => `--file '${f}'`).join(" ")}`;
}
function buildTestCommand(files) {
  if (files.length === 0) return null;
  return `just test ${files.map((f) => `--file '${f}'`).join(" ")}`;
}
function runCommand(cmd, projectDir) {
  try {
    execSync2(cmd, { cwd: projectDir, stdio: ["pipe", "pipe", "pipe"], encoding: "utf-8" });
...
function runQaChecks(qaFiles, projectDir) {
...
function formatStopHookOutput(result) {
