...
3. Create `src/audit/registry.ts` using lazy thunks (NOT a static descriptor array):
   ...
   export const auditRegistry = new Map<AuditKind, AuditDescriptorLoader>([
     [
       "package-surface",
       async () => (await import("#audit/package-surface.js")).packageSurfaceDescriptor,
     ],
...
5. In `src/mcp/tools/audit.ts`, replace the `switch(kind)` body (lines 92-418) with:
   ...
   if (!load) {
     return {
       passed: false,
       summary: `Unknown audit kind: ${kind}`,
       kind,
       details: `No descriptor registered for "${kind}".`,
     };
   }
...
- Adding new audit kinds (including `bucket-boundary` — the wedge anchor names it as the _consumer pull_ that motivates this refactor, but wiring it is a separate blueprint).
