...
- [ ] Failure-path test asserts the _current_ error-message shape (stdout included), not a hypothetical redacted shape
- [ ] `./bin/wp test --file src/runtime/secret-managers.test.ts` passes
      **Note:** Whether secret-provider stdout _should_ appear in error text is a real concern but is a **behavior change**, not a coverage task. It belongs in a separate scoped blueprint (`harden-secret-manager-error-handling`). This task locks current behavior so that change is a visible diff later.
...
**Status:** todo
**Depends:** None
Focus on this file's own logic (server startup, sentinel lifecycle, shutdown). Do not re-test the imported `isDirectEntrypoint`/sentinel utilities where they have their own coverage — assert that `cli.ts` _wires_ them correctly, not their internal behavior.
**Files:**
...
| Task 1.1 might be misread as a behavior-change task          | Acceptance explicitly locks _current_ error-message shape (stdout included); any redaction work is a separate blueprint.                             |
