...
**Acceptance:**
- [ ] All 7 questions answered with explicit decision + rationale.
- [ ] Platform API contract sketched (endpoint shapes, auth header format, error codes).
- [ ] Monorepo boundary confirmed.
- [ ] **Per-repo auth scoping:** auth token is scoped to the calling repo; platform API rejects cross-repo access. Document the enforcement mechanism (JWT claim, signed repo ID, etc.).
- [ ] `notes/design-decisions.md` committed alongside this blueprint.
...
**Acceptance:**
- [ ] `BlueprintPlatformClient` interface defines at minimum: `pushEvent`, `getSnapshot`, `listTemplates`.
- [ ] **Idempotency (per CEO review 2A):** every event payload includes `eventId: string` (UUID generated by client). Platform contract: duplicate `eventId` for the same repo returns 200 without re-applying the event.
- [ ] Event payload type covers all 8 mutation operations.
- [ ] Platform team has reviewed the contract (async; can be in-progress).
- [ ] `pnpm lint:pkg` (attw) passes — new `./blueprint/sync/types` subpath export.
- [ ] `notes/emergency-rollback.md` created documenting the `AK_BLUEPRINT_PLATFORM_DISABLED=1` recovery procedure (per CEO review 1A).
...
**Acceptance:**
- [ ] `pushEvent` writes to platform and returns snapshot.
- [ ] `pushEvent` generates a UUID `eventId` per call (per CEO review 2A); idempotent on retry.
- [ ] Offline behavior matches Q1 decision (buffer or reject with clear error).
- [ ] `AK_BLUEPRINT_PLATFORM_DISABLED=1` bypasses platform writes; falls back to markdown-canonical mode (per CEO review 1A).
- [ ] 429 rate-limit response triggers exponential backoff (NOT immediate retry); surfaces as offline behavior after max retries exceeded.
- [ ] Structured log on every `pushEvent`: `{level, eventType, eventId, httpStatus, durationMs}` (per CEO review 8A).
- [ ] Local counter for consecutive sync failures stored in replica schema; `ak blueprint show` surfaces 'Last synced: <ts>' and 'Sync failures (last hour): N'.
- [ ] Credentials loaded from env var (never hardcoded).
- [ ] **getSnapshot() tests (per eng review 3A):**
  - [ ] Happy path: 200 response → `ingestAll()` called; local replica reflects snapshot.
  - [ ] 404 (repo not found) → clear error thrown; replica unchanged.
  - [ ] Connection drop / JSON parse error → error thrown; replica unchanged.
- [ ] **auth.loadCredential() tests (per eng review 3A):**
  - [ ] Env var present → token returned.
  - [ ] Env var missing → throws with clear error message before any `fetch` call.
  - [ ] Env var empty string → treated as missing; same error.
- [ ] Tests use `vi.stubGlobal('fetch', vi.fn())` — no MSW dependency added.
...
**Acceptance:**
- [ ] Staleness check is a local timestamp compare (no network call if fresh).
- [ ] **Single-flight pattern (per CEO review Section 7):** concurrent replica-refresh calls within the same process coalesce to a single pull via a mutex/promise-sharing pattern. No thundering herd when 6 pll agents all expire TTL simultaneously.
- [ ] Pull adds < 200ms to MCP query tools when replica is fresh (unit test with mocked fetch).
- [ ] `AK_BLUEPRINT_REPLICA_TTL_S` env var respected.
...
**Acceptance:**
- [ ] Platform API is the primary write path (markdown edit is derived output).
- [ ] Auto-commit is atomic; never mixes with user's staged changes.
- [ ] `AK_BLUEPRINT_NO_AUTO_COMMIT=1` suppresses commit; file still written.
- [ ] Offline path consistent with Q1 decision.
- [ ] **IRON RULE REGRESSION (eng review):** With `AK_BLUEPRINT_PLATFORM_DISABLED=1`, `ak_blueprint_task_advance` produces byte-identical markdown output and SQLite state as the pre-sync markdown-canonical implementation. Verified via a golden-fixture snapshot test or before/after state comparison.
...
**Acceptance:**
- [ ] Platform is primary write path.
- [ ] Directory move reflected in platform snapshot + local replica.
- [ ] Auto-commit includes the directory rename.
...
**Acceptance:**
- [ ] Platform is primary write path.
- [ ] Blueprint marked `completed` in platform + local replica.
- [ ] Markdown moved to `blueprints/completed/` as derived output + auto-committed.
...
**Acceptance:**
- [ ] New blueprint registered in platform before markdown is written.
- [ ] Markdown scaffold committed as derived output.
...
**Acceptance:**
- [ ] Stale replica triggers pull-on-demand before returning result.
- [ ] No write to platform.
- [ ] TTL respected: fresh replica → no network round-trip.
...
**Acceptance:**
- [ ] Platform API is primary write path.
- [ ] Directory move derived from platform response.
- [ ] Auto-commit.
...
**Acceptance:**
- [ ] Platform API is primary write path.
- [ ] Markdown updated as derived output.
- [ ] Auto-commit.
