---
title: Emergency Rollback — AK_BLUEPRINT_PLATFORM_DISABLED
status: draft
created: 2026-05-12
---

# Emergency Rollback: `AK_BLUEPRINT_PLATFORM_DISABLED=1`
...
```bash
export AK_BLUEPRINT_PLATFORM_DISABLED=1
```

### Process-scoped (single command)

```bash
AK_BLUEPRINT_PLATFORM_DISABLED=1 ak blueprint task complete task-1
```
...
| Template catalog (`listTemplates`) | Returns empty list; `ak blueprint new --template` falls back to local templates only. |
| Health check (`healthCheck`) | Returns `{ ok: false, latencyMs: 0 }` immediately without making a network call. |
| `ak blueprint` CLI mutation commands | Write to local SQLite + markdown files only (pre-sync behavior). |
| `ak audit blueprint-lifecycle` | Reads from local SQLite replica only; platform state is not consulted. |
...
2. Unset the variable:
   ...
   ```bash
   unset AK_BLUEPRINT_PLATFORM_DISABLED
   ```
...
working without degradation.  `AK_BLUEPRINT_PLATFORM_DISABLED=1` is the
...
- `AK_BLUEPRINT_PLATFORM_URL` — overrides the default platform-api base URL
  (useful for pointing at a staging environment).
- `AK_BLUEPRINT_REPLICA_TTL_S` — controls how often the local replica is
