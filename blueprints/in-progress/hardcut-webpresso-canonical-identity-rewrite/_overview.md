...
- [ ] Root package name is `webpresso`.
- [ ] Release workflow no longer references GitHub Packages publish or
  `scripts/publish-webpresso.ts`.
- [ ] Migration-notice path is removed.
- [ ] `wp_lint` and `wp_typecheck` pass for changed files.
...
- [ ] Marketplace package name is `webpresso`.
- [ ] Plugin manifest name is `webpresso`.
- [ ] Plugin install ID is `webpresso@webpresso`.
- [ ] MCP server ID is `webpresso`.
- [ ] Skill namespace references use `/webpresso:*`.
- [ ] State-root key is `webpresso`.
...
- [ ] All `packages/agent-*` workspaces are deleted.
- [ ] No live code path expects helper-package installation.
- [ ] Workspace config no longer references the deleted helper packages.
- [ ] `wp_lint` and `wp_typecheck` pass for changed files.
...
- [ ] Live install/setup docs reference only `webpresso`.
- [ ] Live skill/plugin docs reference only `/webpresso:*`.
- [ ] `MIGRATION.md` is removed.
- [ ] Docs checks pass.
...
- [ ] Self-host detection uses `webpresso`.
- [ ] Public API surface comments/tests use `webpresso`.
- [ ] `wp_lint` and `wp_typecheck` pass.
...
- [ ] Dev-link paths target `webpresso`.
- [ ] Generated init/plugin output targets `webpresso`.
- [ ] `wp_lint` and `wp_typecheck` pass.
...
- [ ] Live repo metadata and manifests target `webpresso/webpresso`.
- [ ] A grep gate protects against reintroducing the old live slug.
- [ ] `wp_lint` and `wp_typecheck` pass.
...
- [ ] `wp_test` passes.
- [ ] `wp_qa` passes.
- [ ] Blueprint lifecycle check passes.
- [ ] Release dry-run path passes for canonical `webpresso`.
...
- [ ] External npm-registry cleanup steps are recorded.
- [ ] External repo-rename steps are recorded.
- [ ] No compatibility-code fallback is introduced.
