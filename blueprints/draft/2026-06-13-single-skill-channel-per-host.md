...
| F5 | **codex-cli 0.139.0 verified:** `codex plugin marketplace add <local-path\|owner/repo\|git-url>` + `codex plugin add <PLUGIN>@<MARKETPLACE> [--json]` (also `--marketplace`); `codex plugin list/remove`. Disable via `~/.codex/config.toml [plugins."agent-kit@webpresso"] enabled=false`. No `codex plugin install` verb. | Non-interactive install IS available (de-risks the docs-only F5 worry). Selector: `agent-kit@webpresso`. Verify openai/codex#22078 (local-marketplace skills exposure) does not reproduce on 0.139.0 during Task 4. |
