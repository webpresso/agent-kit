...
progress: '100% (11/11 tasks done; workspace pre-flight, manifest/cost/extractor foundations, variant runner, scenario fixtures, CLI/report path, reproducibility test, and docs are implemented and re-verified on 2026-05-29)'
depends_on: []
tags:
  - benchmarking
  - session-memory
...
```webpresso-evidence-v1
[{"command":"wp_test files=scripts/bench/lib/variant-runner.test.ts","exit_code":0,"kind":"test","result":"pass","ts":"2026-05-29T11:00:00Z"},{"command":"wp_lint files=scripts/bench/lib/variant-runner.ts,scripts/bench/lib/variant-runner.test.ts","exit_code":0,"kind":"test","result":"pass","ts":"2026-05-29T11:00:10Z"},{"command":"wp_typecheck cwd=/Users/ozby/repos/webpresso/agent-kit","exit_code":0,"kind":"test","result":"pass","ts":"2026-05-29T11:00:10Z"}]
```
...
- [x] Spawns claude with correct `--plugin-dir` per variant
- [x] Uses per-variant API key from env
- [x] Rate-limit produces clean failure, not corrupt transcript
- [x] Smoke test coverage exists through the mocked/fixture-backed runner tests
...
```webpresso-evidence-v1
[{"command":"wp_test files=scripts/bench/scenarios/_schema.test.ts","exit_code":0,"kind":"test","result":"pass","ts":"2026-05-29T11:00:20Z"},{"command":"wp_lint files=scripts/bench/scenarios/_schema.ts,scripts/bench/scenarios/_schema.test.ts","exit_code":0,"kind":"test","result":"pass","ts":"2026-05-29T11:00:25Z"},{"command":"wp_typecheck cwd=/Users/ozby/repos/webpresso/agent-kit","exit_code":0,"kind":"test","result":"pass","ts":"2026-05-29T11:00:25Z"}]
```
...
- [x] All 3 scenarios validate against zod schema
- [x] qrels file exists and is wired into the checked-in benchmark fixture surface
- [x] Scenario metadata and schema are checked into the repo for deterministic use
...
```webpresso-evidence-v1
[{"command":"wp_test files=src/cli/commands/bench/session-memory.test.ts scripts/bench/lib/report-writer.test.ts","exit_code":0,"kind":"test","result":"pass","ts":"2026-05-29T11:00:35Z"},{"command":"wp_lint files=src/cli/commands/bench/session-memory.ts,src/cli/commands/bench/session-memory.test.ts,scripts/bench/lib/report-writer.ts,scripts/bench/lib/report-writer.test.ts,scripts/bench/README.md,docs/bench/session-memory-methodology.md","exit_code":0,"kind":"test","result":"pass","ts":"2026-05-29T11:00:45Z"},{"command":"wp_typecheck cwd=/Users/ozby/repos/webpresso/agent-kit","exit_code":0,"kind":"test","result":"pass","ts":"2026-05-29T11:00:45Z"}]
```
...
- [x] `--dry-run` validates env without API calls
- [x] Report markdown contains cost, recall@5, wall_sec per cell
- [x] Run-id generation is deterministic from the manifest content hash
- [x] Aborts cleanly if manifest verification fails
...
```webpresso-evidence-v1
[{"command":"wp_test files=scripts/bench/__tests__/reproducibility.test.ts","exit_code":0,"kind":"test","result":"pass","ts":"2026-05-29T11:00:55Z"}]
```
...
- [x] Two runs with same seed → byte-identical transcripts
- [x] Two runs with different seeds → different transcripts (assert diff is non-empty)
...
```webpresso-evidence-v1
[{"command":"wp_audit kind=docs-frontmatter","exit_code":0,"kind":"audit","result":"pass","ts":"2026-05-29T11:01:05Z"},{"command":"wp_lint files=scripts/bench/README.md,docs/bench/session-memory-methodology.md","exit_code":0,"kind":"test","result":"pass","ts":"2026-05-29T11:01:05Z"}]
```
...
- [x] README has working "how to run" example
- [x] Methodology doc cross-references `docs/research/2026-05-14-token-savings-benchmark-methodology.md`
- [x] `wp_audit kind=docs-frontmatter` passes on new docs
...
```webpresso-evidence-v1
[{"command":"bun test scripts/bench/lib/manifest.test.ts","exit_code":0,"kind":"test","result":"pass","ts":"2026-05-29T11:01:15Z"},{"actor":"assistant","allow_manual":true,"description":"Confirmed workspace-mode preflight is enforced through `BENCH_WORKSPACE_MODE` checks in the manifest/workspace validation layer and documented in `scripts/bench/PREFLIGHT.md` and `scripts/bench/README.md`.","kind":"manual","log_excerpt":"Current tests assert the harness refuses to run when workspace mode is unspecified and labels single-workspace mode with the expected cache disclaimer; `PREFLIGHT.md` documents both isolated and single-workspace modes.","result":"pass","ts":"2026-05-29T11:01:15Z"}]
```
...
**Acceptance:**
- [x] Pre-flight refuses to run if workspace mode unspecified
- [x] `single-workspace` mode tags reports with cache disclaimer
- [x] `isolated` mode validates each key resolves to a distinct workspace via the configured verification path
