...
- `vp exec vitest run src/browser/runtime.test.ts src/cli/commands/browser.test.ts` — pass
- `vp run typecheck` — pass
- `wp lint --file src/browser/runtime.ts --file src/browser/runtime.test.ts --file src/cli/commands/browser.ts --file src/cli/commands/browser.test.ts` — pass
- Targeted `wp format --check --file ...` for changed browser/docs/skill/blueprint files — pass
- Whole-repo `wp format --check` — blocked by pre-existing unrelated formatting drift in `src/cli/commands/init/scaffolders/agent-hooks/index.test.ts`
- `wp sync --check` — blocked by pre-existing generated-surface drift (first entry: `.agent/rules/agent-guide.md`; 124 additional entries)
