...
- **New user-visible capability:** An agent running against the webpresso MCP server can call `wp_session_execute` / `wp_session_batch_execute` and have an injected payload (`; rm -rf /`, `$(…)`, a `cwd` outside the repo) rejected with a structured error _before_ anything runs — instead of silently executing it.
