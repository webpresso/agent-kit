---
type: blueprint
title: Harden shared SQLite WAL safety for multi-window agent workloads
status: completed
complexity: S
owner: agent-kit
...
progress: '100% (4/4 tasks done; WAL mode, busy_timeout, multi-process safety, overlap regression coverage, and blocking-lane wiring are all present and re-verified on 2026-05-29)'
depends_on: []
...
[{"command":"wp_test files=src/blueprint/db/wal-multiwindow.integration.test.ts src/blueprint/db/migrations.test.ts","exit_code":0,"kind":"test","result":"pass","ts":"2026-05-29T10:31:00Z"},{"command":"wp_lint files=src/blueprint/db/connection.ts,src/blueprint/db/sqlite.ts,src/blueprint/db/wal-multiwindow.integration.test.ts,src/blueprint/db/migrations.test.ts","exit_code":0,"kind":"test","result":"pass","ts":"2026-05-29T10:31:30Z"},{"command":"wp_typecheck cwd=/Users/ozby/repos/webpresso/agent-kit","exit_code":0,"kind":"test","result":"pass","ts":"2026-05-29T10:31:30Z"}]
```
...
- [x] 10/10 deterministic green on local runs
- [x] Final persisted row count matches expected total writes
- [x] No child process exits with SQLITE_BUSY or lock-related failure
...
- [x] Connection-level overlap behavior is pinned by tests
- [x] WAL mode is asserted explicitly, not assumed
...
- [x] All new WAL/concurrency tests pass
- [x] Any fix is localized to the shared SQLite owner files
- [x] No unrelated persistence/API work is bundled in
...
- [x] WAL tests are part of a blocking verification lane
- [x] CI fails if shared SQLite concurrency regresses
