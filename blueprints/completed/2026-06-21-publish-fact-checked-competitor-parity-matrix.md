...
# Publish fact-checked 5-competitor context-management feature parity matrix

**Goal:** Create `docs/competitive/feature-parity-matrix.md` — a competitive
analysis comparing agent-kit's context-management surface against `context-mode`
(mksglu), `headroom` (chopratejas), `RTK` (rtk-ai), and `OpenCode DCP` — with
every agent-kit claim fact-checked against repo source. The matrix was drafted in
a prior session but lived only as an untracked file; this blueprint carries the
**corrected, review-cleared** version (codex + eng + DX review) so it lands as a
durable, accurate artifact rather than a marketing doc with unverified numbers.

## Product wedge anchor

- **Stage outcome:** Public competitive positioning for the open-sourcing goal
  (workspace `CLAUDE.md` → "the reference target for 'does this work for a 3rd
  party'"). A 3rd-party evaluator deciding between agent-kit and the incumbent
  context-management tools needs a claim-by-claim comparison whose agent-kit
  rows are verifiable, not aspirational.
- **Consuming surface:** `docs/competitive/feature-parity-matrix.md` (the doc
  itself) and its §10 Tier 1/2/3 roadmap, which feeds future agent-kit
  improvement blueprints (RRF fusion, proxy mode, test-runner breadth).
- **New user-visible capability:** A reader (maintainer or 3rd-party evaluator)
  can compare agent-kit vs four competitors on reduction mechanism, search,
  continuity, performance, security, and tool surface, with agent-kit claims
  traceable to `file:line` in this repo and competitor claims marked as
  point-in-time snapshots.

## Context for the executor

The matrix does **not** exist on `main` — it was never committed. The full
**corrected** content is embedded verbatim in Task 1 below; the executor writes
that content exactly. All fact-check corrections (codex-verified) and review
decisions (eng + DX) are already applied in the embedded copy. Do not re-derive
the numbers — they were verified against source on 2026-06-21:

- IDF-Levenshtein is **per-backend**: TS `src/session-memory/store.ts:577-578`
  uses `idfWeight=1+log(1+N/tokens)`, `score=idfWeight/(1+dist)`; Rust native
  `native/session-memory-engine/crates/session-memory-core/src/store.rs:905`
  uses plain `1.0-(dist/max_len)` with no IDF. Never state "IDF-weighted
  Levenshtein" unconditionally.
- `<0.5ms` capture_event is a code-comment **target** (`session.rs`); the
  enforced bench gate (`check-bench-thresholds.sh`) allows up to **2ms**.
- MCP tool count: `src/mcp/tools/` is **~28 compiled** tools; blueprint MCP
  server tools push the total over 30. "30+ audits" is verified for the CLI
  surface (`REPO_AUDIT_REGISTRY` in `src/cli/commands/audit.ts`).
- Competitor stars / downloads / production-token figures are point-in-time
  snapshots (2026-06-21); architecture/algorithm claims are README-sourced and
  stable.
...
Full check (mirrors Tasks 2-4):

```bash
M=docs/competitive/feature-parity-matrix.md
grep -c "IDF-weighted Levenshtein fallback" "$M"   # expect 0 (split wording instead)
grep -q "bench gate" "$M" && echo "capture_event caveat OK"
grep -q "~28 compiled" "$M" && echo "MCP count hedged OK"
grep -qi "projected" "$M" && echo "positioning hedged OK"
grep -qi "adoption lever\|adoption wedge" "$M" && echo "proxy-mode DX note OK"
grep -qi "accessibility gate" "$M" && echo "test-runner DX note OK"
grep -qi "actionable failure output" "$M" && echo "failure-output item OK"
grep -qi "point-in-time\|snapshot" "$M" && echo "data-freshness footnote OK"
```
...
*(Platform scope is intentionally capped at 4 — Claude Code + Codex (Tier 1),
...
gap. Focus below is efficiency, quality, and feature depth.)*

> **DX read on this roadmap:** the single highest **adoption** lever here is
> **proxy/HTTP mode (#6)** — it is the only item that changes *who can adopt*
...
| # | Gap | Competitor with it | What to build | Impact |
|---|-----|-------------------|---------------|--------|
| 6 | **Proxy/HTTP mode** *(top adoption lever — zero-code wedge)* | headroom, RTK | `wp proxy`: OpenAI-compatible HTTP proxy with session-memory middleware + auto-elision. ~500 LOC TS. | Zero-code integration for any OpenAI client — changes *who can adopt* |
...
| 10 | **Test-runner filter breadth** *(accessibility gate for polyglot consumers)* | RTK (8 runners) | Add jest, pytest, cargo-test structured failure parsers. ~100 LOC each. | Non-vitest repos currently get silent passthrough / no savings |
...
| 15 | **Actionable failure output** *(new — DX: fight uncertainty)* | (none — agent-kit lead) | Make compacted lint/test `failures[]` carry problem + cause + fix (+ doc link where available), so an agent consuming compacted output fixes the right thing. ~300 LOC TS. | Failure representation quality decides agent fix accuracy |
...
| ID | Claim | Evidence |
| -- | ----- | -------- |
| C1 | This executable blueprint has a canonical repository document. | repo:blueprints/completed/2026-06-21-publish-fact-checked-competitor-parity-matrix.md |
