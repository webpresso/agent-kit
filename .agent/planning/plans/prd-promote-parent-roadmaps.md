...
## Goal
Make parent roadmaps visible and directly explorable in the `ak` CLI so `/pll` and human operators can see strategic roadmap context instead of orphaned child blueprints only.

## Requirements
1. `ak blueprint list` must no longer silently hide `type: parent-roadmap` documents.
...
3. Add an `ak roadmap` command with `list` and `show` surfaces reusing existing blueprint internals.
...
- Manual CLI probes show roadmaps in list output and allow `ak roadmap show <slug>`.
