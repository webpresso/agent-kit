//! napi-rs FFI bindings for ctx-rs-core.
//!
...
thread_local! {
    static STORE_CACHE: RefCell<HashMap<String, ctx_rs_core::store::Store>> =
        RefCell::new(HashMap::new());
}

// One `SessionEngine` per db_path, per Node.js thread.
thread_local! {
    static SESSION_CACHE: RefCell<HashMap<String, ctx_rs_core::session::SessionEngine>> =
        RefCell::new(HashMap::new());
}

// (db_path, session_id, event_id, tool_name, content)
type PendingEvent = (String, String, String, String, String);
...
fn flush_buffered_events(buf: &mut VecDeque<PendingEvent>) -> NapiResult<()> {
    ...
    while let Some((db_path, session_id, event_id, tool_name, content)) = buf.pop_front() {
        SESSION_CACHE.with(|cache| {
            let mut map = cache.borrow_mut();
            if !map.contains_key(&db_path) {
                let path = Path::new(&db_path);
                let engine = ctx_rs_core::session::SessionEngine::open(path)
                    .map_err(|e| napi::Error::from_reason(e.to_string()))?;
                map.insert(db_path.clone(), engine);
            }
            map.get(&db_path)
                .unwrap()
                .capture_event(&session_id, &event_id, &tool_name, &content)
                .map_err(|e| napi::Error::from_reason(e.to_string()))
        })?;
    }
    Ok(())
...
pub fn index(
    ...
    STORE_CACHE.with(|cache| {
        ...
        if !map.contains_key(&db_path) {
            let path = Path::new(&db_path);
            let s = ctx_rs_core::store::Store::open(path)
                .map_err(|e| napi::Error::from_reason(e.to_string()))?;
...
pub fn capture_event(
    ...
    EVENT_BUFFER.with(|buf| {
        ...
        b.push_back((db_path, session_id, event_id, tool_name, content));
        if b.len() >= 50 {
...
#[napi]
pub fn snapshot(db_path: String, agent_id: String, max_ms: u32) -> NapiResult<SnapshotResult> {
    // Flush pending events so snapshot sees all of them.
    EVENT_BUFFER.with(|buf| flush_buffered_events(&mut buf.borrow_mut()))?;
    ...
    SESSION_CACHE.with(|cache| {
        ...
        let result = map
            .get(&db_path)
            .unwrap()
            .snapshot(&agent_id, max_ms as u64)
            .map_err(|e| napi::Error::from_reason(e.to_string()))?;
...
pub fn restore(
    ...
    SESSION_CACHE.with(|cache| {
        ...
        let events = map
            .get(&db_path)
            .unwrap()
            .restore(&agent_id, &query, limit as usize)
            .map_err(|e| napi::Error::from_reason(e.to_string()))?;
...
pub async fn fetch_and_index(db_path: String, url: String) -> NapiResult<FetchResult> {
    ...
    tokio::task::spawn_blocking(move || {
        ...
        let mut s = ctx_rs_core::store::Store::open(path).map_err(|e| e.to_string())?;
...
pub struct ExecuteResult {
    ...
    pub indexed: bool,
    /// First lines of output, truncated to 500 chars.
    pub summary: String,
}

/// Run a shell command and index output that exceeds 2 KB into the FTS5 store.
///
/// Streams stdout and stderr line-by-line — no full-output memory spike.
...
pub async fn execute_sandboxed(
    ...
    let result = execute_and_index(&path, &command, &label)
        .await
        .map_err(|e| napi::Error::from_reason(e.to_string()))?;
