...
use ctx_rs_core::session::{capture_event, restore, snapshot};
use ctx_rs_core::store::{Store, open};
...
fn bench_snapshot(c: &mut Criterion) {
    ...
    c.bench_function("snapshot_50_events", |b| {
        b.iter(|| {
            snapshot(&conn, agent_id, 5000).unwrap();
        })
    });
}
...
fn bench_restore(c: &mut Criterion) {
    ...
    c.bench_function("restore_100_events", |b| {
        b.iter(|| {
            restore(&conn, agent_id, "agent-kit session", 10).unwrap();
        })
    });
}
