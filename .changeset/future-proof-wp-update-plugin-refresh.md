...
Fix `wp update` OMC refreshes by using the marketplace-qualified `oh-my-claudecode@omc` plugin id, treat optional host/plugin integration refresh failures as warnings after the core package refresh succeeds, and stop scaffolding exact Node patch pin files so repos use the system-wide Node runtime.
