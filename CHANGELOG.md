...
- b1f3060: Route MCP blueprint creation through the shared `BlueprintCreationService`, validate generated drafts before returning them, and keep Vite+ lint/format execution behind the `vp lint`/`vp fmt` facade instead of direct `oxlint`/`oxfmt` commands.
...
- 6ece19c: Add explicit WP-managed optional agent-tool lifecycle commands for Codex, Claude Code, OpenCode, and Oh My integrations; ownership records now drive advisory `wp update` refreshes, strict argument validation, and non-destructive `wp remove` copy.
