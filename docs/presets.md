...
**Idempotency:** OMX manages its own state — every `wp setup` re-invokes `omx setup --yes --scope user`, which is itself idempotent. After OMX finishes, webpresso also migrates deprecated Codex config entries from `[features].codex_hooks` to `[features].hooks` in `$CODEX_HOME/config.toml` (or `~/.codex/config.toml`) so older OMX releases do not keep re-triggering Codex's deprecation warning.
