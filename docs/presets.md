...
1. **Tier-3 skills** — opt-in skill packs from the webpresso catalog (e.g. `tanstack-query`, `react-doctor`, `frontend-design`). Run `wp skill list` for the full list.
...
Every non-`--dry-run` setup run writes webpresso's Codex hook definitions and then
tries to trust only those webpresso-owned hooks through the local Codex runtime.
...
verification still shows untrusted/disabled webpresso hooks, setup prints a
...
**Idempotency:** OMX manages its own state — every `wp setup` re-invokes `omx setup --yes --scope user`, which is itself idempotent. After OMX finishes, webpresso also migrates deprecated Codex config entries from `[features].codex_hooks` to `[features].hooks` in `$CODEX_HOME/config.toml` (or `~/.codex/config.toml`) so older OMX releases do not keep re-triggering Codex's deprecation warning. When project-local Codex hooks already exist in `.codex/hooks.json`, webpresso also converges its owned hook commands in place: stale legacy `ak-*` webpresso hook commands are removed, canonical `wp-*` commands are rewritten to the current managed form, and unrelated third-party/custom hooks are preserved.
**Codex/OMX MCP persistence:** after `omx setup --yes --scope user`, webpresso upserts the owned `[mcp_servers.playwright]` block in `$CODEX_HOME/config.toml` or `~/.codex/config.toml`. This gives both Codex and OMX a persistent Playwright MCP server for browser testing without relying on session-local tool state.
**Side-effects:** OMX writes user-scoped Codex/OMX configuration and may persist `.omx/setup-scope.json` in the consumer repo. `wp setup` also repairs the managed `.gitignore` block for generated agent surfaces (`.codex/`, `.omx/`, `.agent/`, IDE projections) so re-running setup does not expose regenerated files as untracked. When default user-scoped setup migrates a repo that was previously project-scoped, webpresso removes Git-tracked `.codex/` and `.omx/` files so repo-scoped OMX/Codex artifacts do not remain committed; untracked local runtime files are preserved. webpresso also writes the global Codex Playwright MCP block described above, normalizes owned project-local Codex hook entries in `.codex/hooks.json`, and applies the one-line `codex_hooks` → `hooks` feature-flag migration when needed, preserving unrelated config.
...
**Install path:** webpresso uses OMC's Claude Code plugin marketplace path. Upstream OMC also documents an npm runtime package (`oh-my-claude-sisyphus`), but `wp setup` does not use that path. Default setup runs:
...
**Failure/skip modes:**
- `claude` is not on `PATH` → setup warns and skips OMC, because webpresso only drives the Claude Code plugin path
...
Ensures [gstack](https://gstack.lol/)'s **canonical checkout** exists at `~/.claude/skills/gstack/` ([GitHub](https://github.com/garrytan/gstack)). This runs by default on every `wp setup`. If the directory is missing, setup clones from `https://github.com/garrytan/gstack.git`. When Codex is detected, webpresso runs gstack's official combined host setup (`./setup --host auto --team`) from that same checkout so Claude/team mode and Codex materialization refresh in one setup pass. Without Codex, setup runs `./setup --team`.
...
long-tail shell-tool filtering, while webpresso keeps `wp_*` quality routing.
...
**Telemetry / config:** webpresso invokes RTK with
...
- Codex is treated as a prompt/instructions lane upstream, not a hook-rewrite
  lane, so webpresso does **not** add RTK to `.codex/hooks.json`
...
current.
**Requires:** nothing beyond Node/Bun on PATH. rulesync is bundled with webpresso.
