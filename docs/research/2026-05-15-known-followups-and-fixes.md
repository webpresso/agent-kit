...
For agent-kit's case (auto-sync from `catalog/` → `.codex/`, `.gemini/`, `.cursor/`, etc.), the pattern is intentional and matches `ak sync`'s contract. The drift onto `src/cli/commands/init/*.test.ts` files is NOT auto-sync — it's most likely real in-flight source updates from a parallel author (a separate commit `98c8a12` landed mid-session).
...
- `ak sync` + `scaffoldAgentHooks` produce these files; they're not authored
- Untracking is safe: a fresh `ak setup` will recreate them with current shape
- Aligns with AGENTS.md safety boundary explicitly: "agent surfaces… are gitignored and regenerated by `ak setup` / `omx setup`"
...
- Best path forward: keep `CODEX_BIN` absolute (correct for codex trust); gitignore the regenerated file; consumers regenerate fresh on `ak setup`
...
- For agent-kit specifically this is fine — `ak setup` regenerates them
- For consumers vendoring agent-kit, they'd run `ak setup` post-install; same outcome
...
agent-kit's stated principle: catalog is the source of truth, per-IDE surfaces are projected by `ak sync`. Untracking `.codex/agents/` + `.codex/skills/` enforces that principle in git too. Currently the index claims those files are "real" tracked artifacts; after the fix, the index claims what AGENTS.md says: they're regenerated.
...
2. **Untrack and gitignore** `.codex/hooks.json` — its post-8a31e2a content is machine-specific (absolute paths with username) and matches the AGENTS.md safety boundary for "regenerated by `ak setup`".
...
- If a downstream consumer (ozby/ingest-lens) depends on the tracked `.codex/agents/` content → coordinate the cleanup with their `ak setup` flow
