...
| ------------- | -------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `draft`       | `blueprints/draft/<slug>/_overview.md` (folder), or legacy flat `blueprints/draft/<slug>.md`             | Freshly created. Still being scoped. May reference unverified claims. Not ready to execute.                                                                        |
| `planned`     | `blueprints/planned/<slug>/_overview.md` (folder), or legacy flat `blueprints/planned/<slug>.md`         | Refined, fact-checked, task graph ready. Can be picked up by an agent. Tasks haven't started.                                                                      |
| `in-progress` | `blueprints/in-progress/<slug>/_overview.md` (folder), or legacy flat `blueprints/in-progress/<slug>.md` | At least one task has started. Progress is tracked in the canonical blueprint markdown frontmatter (`progress:` field) and via per-task `**Status:**` annotations. |
| `completed`   | `blueprints/completed/<slug>/_overview.md` (folder), or legacy flat `blueprints/completed/<slug>.md`     | Every task marked `done`. Acceptance criteria ticked. Ready for archival.                                                                                          |
| `archived`    | `blueprints/archived/<slug>/_overview.md` (folder), or legacy flat `blueprints/archived/<slug>.md`       | Historical record. Read-only.                                                                                                                                      |
| `parked`      | `blueprints/parked/<slug>/_overview.md` (folder), or legacy flat `blueprints/parked/<slug>.md`           | Paused indefinitely. Reason captured in frontmatter. Resumes into `planned/` or `in-progress/`.                                                                    |
...
- **`draft → planned`** requires the plan pass format audit (`wp blueprint audit <slug> --strict`), ideally `/plan-refine`, **and ≥2 approvals from distinct reviewers** in frontmatter `approvals:`. Enforced by `wp blueprint move`/`promote <slug> planned` — the command refuses if the audit fails OR fewer than 2 distinct provenance-backed reviewer approvals are present. Each counted approval must have a matching committed `reviews.md` structured entry that references a separate tracked review transcript/artifact. (`wp audit blueprint-lifecycle` also flags legacy missing-approval cases.) See `catalog/agent/rules/pre-implementation.md`.
