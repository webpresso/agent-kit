...
const HOST_POLICIES = {
    claude: {
        ...
        stdoutNoop: 'SessionStart always writes a JSON additionalContext envelope; an empty project routing file still emits the shared routing source.',
        lifecycleNotes: [
...
export function routingToolNamesFromSource(content) {
    return [...content.matchAll(/<tool name="([^"]+)">/gu)].map((match) => match[1]);
}
function renderHostEnvelope(host, policy, sourceName, routingContent) {
