const ROUTING_RULES = [
    ...
    },
    {
        ...
        guidanceType: 'qa',
        guidance: 'Use wp_worker_tail MCP tool instead — secret-aware Worker tail output is bounded, redacted, and routed through the shared secret-provider gate',
        tool: 'wp_worker_tail',
    },
    {
        prefixes: [
...
function getRuleDirectToolBins(prefixes) {
    ...
    for (const prefix of prefixes) {
        ...
        if (command && !PACKAGE_MANAGER_PREFIXES.has(command))
            bins.add(command);
    }
    return bins;
}
