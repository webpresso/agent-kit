...
export function validateWorktreeDiscipline(input) {
    ...
    if (!op)
        return { validator: VALIDATOR_NAME, passed: true };
    const cwd = input.cwd ?? "";
    if (!isPrimaryReposCheckout(cwd))
        return { validator: VALIDATOR_NAME, passed: true };
    return {
        validator: VALIDATOR_NAME,
        passed: false,
        message: `"${op}" in a primary checkout (${cwd}). Primary ~/repos checkouts stay on main; ` +
...
function isPrimaryReposCheckout(cwd) {
    if (!cwd)
        return false;
    if (cwd.includes("/.agent/worktrees/"))
        return false;
...
function forbiddenGitOp(command) {
    if (!/\bgit\b/.test(command))
        return null;
    if (/\bgit\s+commit\b/.test(command))
        return "git commit";
    if (/\bgit\s+switch\b(?!\s+(?:-h|--help)\b)/.test(command))
        return "git switch";
    if (/\bgit\s+checkout\s+-b\b/.test(command))
        return "git checkout -b";
    // branch CREATION only: `git branch <name>`; allow listing/info/delete flags.
    if (/\bgit\s+branch\s+(?!-|--)\S/.test(command))
