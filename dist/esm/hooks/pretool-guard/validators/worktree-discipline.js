 ...
 */
export function validateWorktreeDiscipline(input) {
    if (process.env.WORKTREE_DISCIPLINE_SKIP === "1")
    ...
    for (const op of forbiddenGitOps(command)) {
        ...
        if (effective.ambiguous)
            return blocked(op.label, input.cwd ?? "", true);
        if (isPrimaryReposCheckout(effective.cwd))
            return blocked(op.label, effective.cwd, false);
    }
    if (hasAmbiguousGitMutationSyntax(command))
        return blocked("git", input.cwd ?? "", true);
    return { validator: VALIDATOR_NAME, passed: true };
}
function blocked(op, cwd, ambiguous) {
    ...
    return {
        ...
        passed: false,
        message: `"${op}" ${where}. Primary ~/repos checkouts stay on main; work in a managed ` +
...
function isPrimaryReposCheckout(cwd) {
    if (!cwd)
        return false;
    const managedWorktreesRoot = `${homedir()}/.agent/worktrees/`;
    if (cwd === `${homedir()}/.agent/worktrees` || cwd.startsWith(managedWorktreesRoot))
        return false;
...
function mutationLabelFromGitArgs(args) {
    for (let i = 0; i < args.length; i += 1) {
        ...
        if (word.startsWith("-C") ||
            ...
            continue;
        if (word === "-p" || word === "--paginate" || word === "--no-pager")
            continue;
        if (word.startsWith("-"))
            continue;
        if (word === "commit")
        ...
        if (word === "checkout")
            return checkoutArgsMutate(args.slice(i + 1)) ? "git checkout" : null;
        if (word === "branch") {
...
function hasUnsupportedGlobalBeforeMutation(args) {
...
function hasEnvTargetOverrideBeforeGit(words, gitIndex) {
...
function hasGitConfigAliasBeforeGit(words, gitIndex) {
    return (words.slice(0, gitIndex).some((word) => word.startsWith("GIT_CONFIG_COUNT=")) &&
        words.slice(0, gitIndex).some((word) => /^GIT_CONFIG_KEY_\d+=alias\./u.test(word)));
}
function wordsContainGitMutation(words) {
    for (let i = 0; i < words.length; i += 1) {
        if (!isGitExecutable(words[i]))
            continue;
        if (hasGitConfigAliasBeforeGit(words, i))
            return true;
        if (mutationLabelFromGitArgs(words.slice(i + 1)))
            return true;
    }
    return false;
}
function segmentContainsGitMutation(segment) {
    const words = shellWords(segment);
    if (words === "ambiguous")
        return /\bgit\b/u.test(segment) && /\b(?:commit|switch|checkout|branch)\b/u.test(segment);
    return wordsContainGitMutation(words);
}
function quotedPayloads(command, prefix) {
    const payloads = [];
...
function payloadContainsGitMutation(payload) {
    return payload.split(/&&|;|\|\||[(){}]/u).some(segmentContainsGitMutation);
}
function hasReinterpretedGitMutation(command) {
    for (const segment of splitShellSegments(command)) {
        ...
        for (let i = 0; i < words.length; i += 1) {
            ...
            if (["bash", "sh", "zsh"].includes(basename(word))) {
                const flagIndex = words.findIndex((candidate, candidateIndex) => candidateIndex > i &&
                    (candidate === "-c" || candidate === "-lc" || candidate.endsWith("c")));
                if (flagIndex !== -1 && payloadContainsGitMutation(words.slice(flagIndex + 1).join(" ")))
                    return true;
            }
            if (word === "eval" && payloadContainsGitMutation(words.slice(i + 1).join(" ")))
                return true;
            if (word === "env") {
                const splitIndex = words.findIndex((candidate, candidateIndex) => candidateIndex > i && candidate === "-S");
...
function checkoutArgsMutate(args) {
    const nonQuietArgs = args.filter((arg) => arg !== "-q" && arg !== "--quiet");
    if (nonQuietArgs.length === 0)
        return false;
    if (nonQuietArgs[0] === "--")
        return false;
    if (nonQuietArgs[0] === "-" || nonQuietArgs.includes("--detach"))
        return true;
    if (nonQuietArgs.some((arg) => arg === "-b" || arg === "-B" || arg === "--orphan" || arg === "--track"))
        return true;
    if (nonQuietArgs.some((arg) => arg.startsWith("-b") || arg.startsWith("-B")))
        return true;
    if (nonQuietArgs.includes("--"))
        return false;
...
function branchArgsMutate(args) {
    if (args.length === 0)
        return false;
    if (args.some((word) => word === "-d" || word === "-D" || word === "--delete"))
        return false;
    if (args.some((word) => BRANCH_ACTION_FLAGS.has(word)))
        return true;
    ...
    for (let i = 0; i < args.length; i += 1) {
        ...
        if (!word.startsWith("-"))
            return !sawReadonly;
        if (BRANCH_READONLY_OR_DELETE_FLAGS.has(word) ||
...
function hasAmbiguousGitMutationSyntax(command) {
    ...
    if (/\bc\\d\b/u.test(command) && /\bgit\b/u.test(command))
        return true;
    // Nested shells/eval/env -S re-interpret code; do not try to model their cwd.
    if (hasReinterpretedGitMutation(command))
        return true;
    for (const payload of quotedPayloads(command, /\b(?:bash|sh|zsh)\b[^;&|(){}]*(?:-c|-lc)\s+(["'])/gu)) {
        if (payloadContainsGitMutation(payload))
            return true;
    }
    ...
    for (const match of command.matchAll(/(?:^|[;&|({]\s*)\$\([^)]*\)([^;&|(){}]*)/gu)) {
        ...
        if (words === "ambiguous" || wordsContainGitMutation(words))
            return true;
    }
    if (/\bgit\s+\$\([^)]*\)/u.test(command))
        return true;
    for (const segment of splitShellSegments(command)) {
        const words = shellWords(segment);
        if (words === "ambiguous")
        ...
        for (let i = 0; i < words.length; i += 1) {
            ...
            if (hasMutationAliasConfig(args.join(" ")))
                return true;
            if (hasGitConfigAliasBeforeGit(words, i))
                return true;
            ...
            if (!mutation)
                continue;
            if (hasEnvTargetOverrideBeforeGit(words, i))
                return true;
            if (args.some((word) => word === "--git-dir" ||
                word === "--work-tree" ||
                word.startsWith("--git-dir=") ||
                word.startsWith("--work-tree=")))
                return true;
            if (hasUnsupportedGlobalBeforeMutation(args))
                return true;
            ...
            if (forbiddenGitOps(segment).length === 0)
                return true;
        }
    }
    return false;
...
function hasUnquotedCwdCommand(command) {
    for (const m of command.matchAll(CWD_WORD)) {
        if (!isInsideSingleOrDoubleQuotes(command, m.index ?? 0))
            return true;
    }
    return false;
}
...
function applyDir(token, cwd) {
    if (!token)
        return { cwd };
    if (isUnresolvable(stripQuotes(token)))
        return "ambiguous";
    return { cwd: resolveDir(token, cwd) };
}
function isTrustedCwdChainStart(commandPrefix, chainStart) {
    if (isInsideSingleOrDoubleQuotes(commandPrefix, chainStart))
        return false;
    if (chainStart === 0)
        return true;
...
function applySuccessGatedCwdChain(commandPrefix, baseCwd) {
    ...
    if (!isTrustedCwdChainStart(commandPrefix, chainStart))
        return { ambiguous: true };
    let cwd = baseCwd;
 ...
 */
function resolveEffectiveCwd(command, baseCwd, opGlobals, opIndex) {
    const fromCwdChain = applySuccessGatedCwdChain(command.slice(0, opIndex), baseCwd);
    if (fromCwdChain.ambiguous)
        return fromCwdChain;
...
const GIT_GLOBAL_RUN = `(?:-C\\s+${QUOTED_ARG}\\s+|-c\\s+${QUOTED_ARG}\\s+|--git-dir(?:=\\S+|\\s+${QUOTED_ARG})\\s+|` +
...
function forbiddenGitOps(command) {
...
//# sourceMappingURL=worktree-discipline.js.map