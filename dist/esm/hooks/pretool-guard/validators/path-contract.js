...
export function isBlueprintPath(filePath) {
...
export function isCanonicalBlueprintOverviewPath(filePath) {
    const normalized = normalizePlanningPath(filePath);
    const parts = normalized.split('/');
...
export function getBlueprintPathViolation(filePath) {
    const normalized = normalizePlanningPath(filePath);
    if (!isBlueprintPath(normalized))
        return null;
