...
import { PROBE_ROWS, assertConformance, } from "#hooks/__conformance__/matrix.js";
...
import { formatRuntimeTypecheckParityFailures, probeRuntimeTypecheckParity, } from "#typecheck/runtime-parity.js";
 ...
 */
function probeDecisionRow(wpCli, row) {
...
function inspectNativePluginRuntime() {
    const root = resolvePluginRoot();
    if (!root) {
        return {
            launchMode: "missing",
...
function runtimePackageNameForTarget(target) {
    if (target.packageName)
        return target.packageName;
    return target.id ? `@webpresso/agent-kit-runtime-${target.id}` : null;
}
function resolveRuntimeBinaryCandidates(root, manifest, target) {
    const packageName = runtimePackageNameForTarget(target);
    if (!target.id || !packageName)
        return [];
...
export function checkNativePluginRuntime() {
    ...
    const isSourceCheckoutSkip = status.launchMode === "native" &&
        status.reason?.startsWith("skipped (source checkout") === true;
    return {
...
export function checkPhase2RuntimeTypecheckParity() {
    const status = inspectNativePluginRuntime();
...
async function checkOpenCodeHost(cwd = process.cwd()) {
    ...
    if (!available.ok) {
        return {
            ...
            ok: true,
            detail: "skipped (opencode not on PATH)",
        };
    }
...
const AGENT_KIT_HOOK_MARKERS = ["bin/wp", " hook pretool-guard"];
...
export function checkPackagedHostArtifacts(cwd = process.cwd()) {
    const missingCodexArtifacts = CODEX_PLUGIN_ARTIFACTS.filter((artifactPath) => !tryAccess(join(cwd, artifactPath)));
    ...
    if (!hasClaudePlugin)
        failures.push("missing Claude artifact: .claude-plugin/plugin.json");
    ...
    if (codexMcp) {
        ...
        if (serverRecord?.command !== "${PLUGIN_ROOT}/bin/wp" ||
            ...
            failures.push("Codex MCP artifact must expose direct webpresso server map launching ${PLUGIN_ROOT}/bin/wp mcp");
        }
    }
    if (codexHooks) {
        const hooks = codexHooks.hooks;
        const hookKeys = hooks && typeof hooks === "object" && !Array.isArray(hooks) ? Object.keys(hooks) : null;
    ...
    const repairHint = "Codex repair: run `wp setup`; package repair: rebuild the public artifact from source";
...
export function checkHostArtifactOwnership(cwd = process.cwd()) {
...
export function checkHostLifecycleDepth() {
    return {
        name: "host lifecycle depth",
        ok: true,
        detail: "Claude/Codex managed hooks: full for replacement-critical lifecycle events; Cursor/OpenCode: degraded with host-specific lifecycle depth",
    };
}
/**
...
export async function printHooksDoctor(opts = {}) {
    ...
    if (!result.ok) {
        ...
        console.error(`  • Refresh local hook/plugin surfaces: \`${setupCommandForRepo(opts.cwd ?? process.cwd())}\``);
        console.error("  • Consumers: run `vp install -g @webpresso/agent-kit && wp setup`; source checkout: run `WP_FORCE_SOURCE=1 wp setup --source-maintenance`.");
