...
function hasValidCodexPluginArtifacts(repoRoot) {
    ...
    if (!existsSync(pluginPath) || !existsSync(mcpPath) || !existsSync(hooksPath))
        return false;
    ...
    return (plugin?.mcpServers === "./codex.mcp.json" &&
...
function supportLevelForHost(host) {
    const emittedEvents = new Set(WP_HOOK_SPECS.map((spec) => spec.event));
    const levels = CAPABILITY_MATRIX.filter((entry) => emittedEvents.has(entry.event)).map((entry) => entry[host]);
    if (levels.length === 0)
        return "unsupported";
...
export function deriveHostSurfaceStatus(repoRoot) {
    return [
        {
            host: "claude",
            ...
            required: true,
            ownership: "plugin artifact owns MCP; active hooks stay setup-managed in .claude/settings.json",
        },
        {
            host: "codex",
            ...
            required: true,
            ownership: "hooks/hooks.json is inert package metadata; active hooks stay in .codex/hooks.json",
        },
        {
            host: "cursor",
        ...
        },
        {
            ...
            required: false,
            ownership: "plugin bridge owns OpenCode integration; unsupported lifecycle events remain explicit",
        },
    ];
}
function countHookEntries(hooksMap) {
    return Object.values(hooksMap).reduce((total, groups) => total + groups.reduce((groupTotal, group) => groupTotal + group.hooks.length, 0), 0);
}
function hookNamesInMap(hooksMap) {
    return WP_HOOK_SPECS.filter((spec) => hookAppearsInMap(hooksMap, spec.hook)).map((spec) => `${spec.event}:${spec.hook}`);
}
function readOpenCodePluginHooks(filePath) {
...
export function deriveHookConfigSourceStatus(repoRoot, homePath = homedir()) {
    return [
...
function gitRelativePath(filePath, repoRoot) {
...
export function formatHookConfigSourceStatusLine(source) {
    ...
    return (`${source.vendor}: scope=${source.scope} status=${source.status} ` +
        `${location} hooks=${hooks}${note}`);
}
export function formatHostSurfaceStatusLine(surface) {
    return (`${surface.host}: artifact=${surface.packagedArtifact} active=${surface.activeHooks} ` +
...
function printHostSurfaceStatus(repoRoot) {
    process.stdout.write("\n[host surfaces]\n");
    process.stdout.write(`${"─".repeat(80)}\n`);
...
function printHookConfigSourceStatus(repoRoot) {
    process.stdout.write("\n[hook sources]\n");
    process.stdout.write(`${"─".repeat(80)}\n`);
...
export async function statusCommand(argv) {
    ...
    if (!manifestExists) {
        process.stdout.write(`No hooks manifest found. Run \`${setupCommandForRepo(repoRoot)}\` to regenerate managed hook state.\n`);
    }
