...
export function syncPerSkillConsumer(repoRoot, config) {
    const skillsSource = join(repoRoot, '.agent/skills');
    if (!existsSync(skillsSource))
        return 0;
    ...
    for (const skill of agentSkills) {
        ...
        if (stats) {
            if (stats.isSymbolicLink()) {
                ...
                console.log(`  ⚠️  ${skill}: real directory collides with .agent/skills/${skill} — skipped (remove manually)`);
                continue;
            }
        }
    ...
    for (const entry of readdirSync(consumerDir)) {
        ...
        if (!target.includes('.agent/skills/'))
            continue;
        const resolvedTarget = resolve(consumerDir, target);
...
export function syncPerSkillConsumers(repoRoot, consumers = DEFAULT_PER_SKILL_CONSUMERS) {
    const skillsSource = join(repoRoot, '.agent/skills');
    if (!existsSync(skillsSource))
        return 0;
    let fixCount = 0;
