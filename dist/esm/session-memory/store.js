...
export class SessionMemoryStore {
    ...
    searchUnified(options) {
        if (options.sourceTypes && !options.sourceTypes.includes('indexed_chunk'))
            return [];
        ...
        return dedupeUnifiedResults(raw.map((result) => this.mapUnifiedResult(result, options.maxPreviewBytes)), limit);
    }
    getChunkById(id) {
        const row = this.db
            .prepare('SELECT id, source, text, metadata_json, created_at FROM session_memory_chunks WHERE id = ?')
            .get(id);
        if (!row)
            return undefined;
        return {
    ...
    getChunksBySource(source) {
        return this.db
            .prepare(`SELECT id, source, text, metadata_json, created_at
    ...
    stats() {
        ...
        const sources = this.db
            .prepare('SELECT DISTINCT source FROM session_memory_chunks ORDER BY source ASC')
            .all()
            .map((row) => row.source);
        return { chunkCount, sourceCount: sources.length, sources };
    }
    recordGainEvent(event) {
        ...
        const baseId = createHash('sha256')
            ...
            .digest('hex')
            .slice(0, 32);
        const insert = this.db.prepare(`INSERT OR IGNORE INTO session_memory_gain_events
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`);
        for (let suffix = 0; suffix < 1_000; suffix += 1) {
            ...
            const result = insert.run(id, event.toolName, event.rawBasisBytes, event.returnedToolResultBytes, event.gainBytes, event.approxTokensSaved, event.rawBytesBasis, event.precision, createdAt);
            if (result.changes > 0)
                return id;
        }
    ...
    purge(options = {}) {
        const ids = options.source
            ? this.db
                .prepare('SELECT id FROM session_memory_chunks WHERE source = ? ORDER BY id ASC')
                .all(options.source)
            : this.db
                .prepare('SELECT id FROM session_memory_chunks ORDER BY id ASC')
                .all();
        ...
        const tx = this.db.transaction((rawIds) => {
            for (const row of rawIds) {
                this.db.prepare('DELETE FROM session_memory_chunks_fts WHERE id = ?').run(row.id);
                this.db.prepare('DELETE FROM session_memory_chunks_tri WHERE id = ?').run(row.id);
                this.db.prepare('DELETE FROM session_memory_chunks WHERE id = ?').run(row.id);
            }
            if (options.source) {
    ...
    deleteNativeSourceTables(source) {
        for (const table of ['chunks', 'chunks_trigram']) {
            if (!this.tableExists(table))
                continue;
            if (source)
                this.db.prepare(`DELETE FROM ${table} WHERE source = ?`).run(source);
            else
        ...
        if (!this.tableExists('sources'))
            return;
        if (source)
            this.db.prepare('DELETE FROM sources WHERE label = ?').run(source);
        else
    ...
    doctor() {
        ...
        if (quickCheck?.quick_check !== 'ok')
            warnings.push('index store quick_check failed');
    ...
    mapUnifiedResult(result, maxPreviewBytes) {
