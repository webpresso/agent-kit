 ...
 */
export function buildVpGlobalInstallCommand(vpCommand = 'vp') {
    return appendGlobalCapableVpArgs(vpCommand, ['install', '-g', PUBLIC_PACKAGE_NAME]);
}
export function buildNpmGlobalInstallCommand(npmCommand = 'npm') {
    return [npmCommand, 'install', '-g', PUBLIC_PACKAGE_NAME];
}
function commandForTopology(topology, resolveVpCommand, resolveNpmCommand) {
    if (topology === 'npm')
        return buildNpmGlobalInstallCommand(resolveNpmCommand());
 ...
 */
export function detect(env, argv0, resolveVpCommand = () => resolveGlobalCapableVpCommand(env.PATH ?? '') ?? resolveBundledVpCommand(), resolveNpmCommand = () => 'npm') {
    ...
    if (fromPath === 'vp' || fromUa === 'vp') {
        ...
        if (command === null)
            return { abort: globalVpUnavailableMessage() };
...
export function matchStoreMarker(realpath) {
    ...
    if (segments.includes('.vite-plus'))
        return 'vp';
    if (isGlobalNodeModules(realpath))
        return 'npm';
    return null;
}
/**
...
function globalVpUnavailableMessage() {
...
export function resolveBundledVpCommand() {
