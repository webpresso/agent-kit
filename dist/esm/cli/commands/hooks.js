...
export function registerHooksCommand(cli) {
    cli
        .command('hooks [subcommand] [...args]', 'Verify plugin hook installation health (subcommands: doctor, status, errors, dispatch, demo, upgrade)')
        .option('--skip-mcp', 'Skip MCP server liveness check (for CI)')
        .option('--probe-decisions', 'Fire the smallest allow/deny conformance rows at pretool-guard and assert the routing decision (doctor only)')
        ...
        switch (subcommand) {
            ...
            case 'errors': {
                ...
                if (options.limit !== undefined)
                    extraArgs.push('--limit', String(options.limit));
