...
export function runGain(options = {}) {
    ...
    if (rtk.status === 0) {
        ...
        if (rtk.stderr.trim())
            console.log(indent(rtk.stderr.trim()));
...
function readWebpressoGainStats(dbPath) {
...
function printWebpressoGain(stats, dbPath) {
    ...
    for (const tool of stats.byTool) {
        console.log(`    ${tool.toolName}: ${tool.gainBytes} bytes, ~${tool.approxTokensSaved} tokens (${tool.eventCount} event${tool.eventCount === 1 ? "" : "s"})`);
    }
}
function printRtkGain(stdout) {
    ...
    try {
        ...
        if (typeof tokens === "number")
            console.log(`  RTK reported tokens saved: ${tokens}`);
        if (typeof bytes === "number")
            console.log(`  RTK bytes saved: ${bytes}`);
