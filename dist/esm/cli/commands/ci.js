import { buildPublicCiActCommand, preparePublicCiActExecution, resolveCiActSecretEnvProfile, sanitizePublicCiActArgv, } from '#ci/act-runner.js';
...
export function registerCiCommand(cli) {
    cli
        ...
        .option('--secret-profile <profile>', 'Repo-owned secret profile from .webpresso/secrets.config.json')
...
export async function runCiActCommand(options = {}, deps = {}) {
    ...
    try {
        ...
        if (prepared.nonSecurityEquivalent) {
            ;
            (deps.stderr ?? process.stderr).write('[wp ci act] replay mode is a generated local approximation and is not security-equivalent to GitHub CI or OIDC.\n');
        }
        if (stdout)
