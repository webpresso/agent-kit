...
import { detectConsumer, isAgentKitTemplateSourceRepo, } from './init/detect-consumer.js';
...
export function isUnmaterializedAgentKitSourceWorktree(consumer) {
...
export function registerSyncCommand(cli) {
    cli
        ...
        if (check && isUnmaterializedAgentKitSourceWorktree(consumer)) {
            console.error(`wp sync --check: agent-kit source surfaces are not materialized in this worktree (${consumer.repoRoot}).\n` +
