...
export function warnIfNonLocalCli(repoRoot, cliUrl = import.meta.url) {
    ...
    const pinnedVersion = ourPkg?.dependencies['@webpresso/agent-kit'] ??
    ...
    if (typeof pinnedVersion !== 'string' || !isPublishedAgentKitRange(pinnedVersion)) {
        console.error('warning: missing or invalid @webpresso/agent-kit dependency pin. ' +
            'Consumers must pin a published semver range in package.json, run `vp install`, then use global `wp setup`.');
    }
}
function isPublishedAgentKitRange(value) {
