...
import { agentKitMcpLaunchCommand, ensureCodexContextModeMcp, findWebpressoMcpEntry, } from '#cli/commands/init/scaffolders/codex-mcp/index';
...
function resolveNodeBinaryOverride(explicit) {
    if (explicit)
        return explicit;
...
function normalizeClaudeContextModeCommand(command, nodeBinary, scriptPath) {
    const trimmed = command.trim();
    const desired = `${quoteShell(nodeBinary)} ${quoteShell(scriptPath)}`;
    if (trimmed === desired)
        return command;
    if (stripOuterQuotes(trimmed) === scriptPath)
        return desired;
...
function normalizeClaudeContextModeHooks(settingsPath, nodeBinary, options) {
    ...
    const hooksRoot = parsed.hooks && typeof parsed.hooks === 'object' && !Array.isArray(parsed.hooks)
        ? { ...parsed.hooks }
        : null;
    ...
    if (!changed)
        return { targetPath: settingsPath, action: 'identical' };
    if (options.dryRun)
        return { targetPath: settingsPath, action: 'skipped-dry' };
    const next = {
        ...parsed,
        hooks: {
            ...hooksRoot,
...
export function patchOpenCodeContextModeConfig(existing, agentKitCommand = ['wp', 'mcp']) {
...
function resolveOpenCodeWebpressoCommand(repoRoot) {
...
export function ensureContextMode(input) {
    ...
    return {
        ...
        claudeGlobalHooks: normalizeClaudeContextModeHooks(claudeSettingsPath, resolveNodeBinaryOverride(input.nodeBinary), input.options),
        opencodeConfig: patchJsonFile(opencodeConfigPath, (existing) => patchOpenCodeContextModeConfig(existing, resolveOpenCodeWebpressoCommand(input.repoRoot)), input.options),
        installed,
    };
}
