...
export function ensureAgentKitGlobal(input) {
    ...
    const vpCommand = input.resolveVpCommand !== undefined ? input.resolveVpCommand() : resolveGlobalCapableVp(env.PATH);
    if (vpCommand === null) {
        return { kind: 'agent-kit-global-skipped-no-vp', hint: NO_VP_HINT };
    }
    const command = [vpCommand, ...buildVpGlobalInstallCommand().slice(1)];
    spinner.start();
