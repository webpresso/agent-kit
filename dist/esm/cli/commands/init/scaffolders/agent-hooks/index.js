...
function sourceRepoHookRuntimePath(repoRoot, platform = process.platform, arch = process.arch) {
    const target = resolveRuntimeTarget(platform, arch);
    if (!target)
        return undefined;
    return join(repoRoot, "bin", "runtime", target.id, runtimeBinaryFilename(target));
}
function sourceRepoAbsoluteBunCandidates(env = process.env, homeDirectory = env.HOME ?? homedir()) {
    const candidates = [
        env.BUN,
...
export function resolveSourceRepoHookLauncher(repoRoot, options = {}) {
    ...
    for (const candidate of sourceRepoAbsoluteBunCandidates(options.env, options.homeDirectory)) {
        if (executable(candidate))
            return { kind: "bun", launcherPath: candidate, cliPath };
    }
    return { kind: "none", cliPath };
}
function buildHookStatusGuard(command, fallback) {
    return `${command}; status=$?; if [ "$status" -eq 2 ]; then exit 2; elif [ "$status" -ne 0 ]; then ${fallback}; fi`;
}
function buildSourceRepoDirectHookCommand(repoRoot, name) {
    ...
    if (runtimePath) {
        ...
        branches.push(`if [ -x ${quotedRuntimePath} ]; then (cd ${repoRootPath} && ${buildHookStatusGuard(`${quotedRuntimePath} hook ${hookName}`, fallback)} );`);
    }
    for (const bunPath of sourceRepoAbsoluteBunCandidates()) {
        const quotedBunPath = quoteShell(bunPath);
        branches.push(`${branches.length === 0 ? "if" : "elif"} [ -x ${quotedBunPath} ]; then (cd ${repoRootPath} && ${buildHookStatusGuard(`${quotedBunPath} ${cliPath} hook ${hookName}`, fallback)} );`);
    }
    const noneBranch = branches.length === 0 ? `if false; then true; else ${fallback}; fi` : `else ${fallback}; fi`;
    return `${branches.join(" ")} ${noneBranch} # ${name}`;
}
function buildConsumerDirectWpHookCommand(repoRoot, name) {
 ...
 */
export function extractWebpressoHookBinName(command) {
