...
function isNodeExecutablePath(execPath) {
...
function resolveNodeOnPath(pathEnv) {
...
export function resolveNodeRuntimeForHookLaunchers(options = {}) {
    ...
    if (isNodeExecutablePath(execPath) && isExecutableFile(execPath))
        return execPath;
...
function resolveGlobalWpLauncher() {
...
function pretoolGuardDeny(reason) {
    return `printf '%s\\n' '${JSON.stringify({
        hookSpecificOutput: {
...
function missingLauncherFallbackCommand(name, launcherPath) {
    if (isJsonOnlyHookBin(name))
        return JSON_ONLY_HOOK_FALLBACK;
    if (name === PRETOOL_GUARD_BIN) {
        return pretoolGuardDeny(`managed hook launcher missing or not executable at ${launcherPath}; run wp setup --restore-hooks or wp setup`);
    }
    return 'true';
}
...
function renderManagedWebpressoHookLauncher(repoRoot, binName) {
    ...
    const missingProjectFallback = binName === PRETOOL_GUARD_BIN
