...
export function normalizeGlobalCodexHooksJson(raw, options, managedHooksDir) {
    ...
    for (const [event, groups] of Object.entries(hooks)) {
        ...
        const catchAllManagedOmxGroupKeys = new Set(normalizedGroups
...
function stableManagedOmxHookGroupKey(group) {
    ...
    return JSON.stringify(hooks.map((hook) => ({
        type: hook.type ?? '',
        command: typeof hook.command === 'string' ? hook.command.trim() : '',
        timeout: hook.timeout ?? null,
    })));
}
function isCatchAllMatcherGroup(group) {
