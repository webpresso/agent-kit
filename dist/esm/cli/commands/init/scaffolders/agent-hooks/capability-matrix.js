...
function hostsMentionedInScope(hostScope, hosts) {
...
export function validateReplacementParityCapabilityCrosswalk(rows, crosswalks = REPLACEMENT_PARITY_CAPABILITY_CROSSWALK) {
    ...
    for (const crosswalk of crosswalks) {
        for (const row of rows.filter((candidate) => candidate.capability.toLowerCase() === crosswalk.capability.toLowerCase())) {
            if (row.status !== undefined && row.status !== 'passed')
                continue;
            if (row.supportLevel !== 'full')
                continue;
