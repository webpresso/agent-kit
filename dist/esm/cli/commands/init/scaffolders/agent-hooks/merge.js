...
function extractAgentKitBinName(command) {
    ...
    if (directClaudeBinMatch !== null)
        return directClaudeBinMatch[1] ?? null;
    const guardedClaudeBinMatch = GUARDED_CLAUDE_NODE_MODULES_BIN_PATTERN.exec(command.trim());
    if (guardedClaudeBinMatch !== null)
        return guardedClaudeBinMatch[2] ?? null;
    const ifGuardedClaudeBinMatch = IF_GUARDED_CLAUDE_NODE_MODULES_BIN_PATTERN.exec(command.trim());
    if (ifGuardedClaudeBinMatch !== null)
        return ifGuardedClaudeBinMatch[2] ?? null;
