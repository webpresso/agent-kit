...
function readExpectedPluginVersion(pluginManifestPath) {
    try {
...
function outputConfirmsExpectedPluginInstalled(stdout, expectedVersion) {
    ...
    try {
        ...
        return (parsed.installed?.some((plugin) => plugin.pluginId === CODEX_PLUGIN_ID &&
            plugin.installed === true &&
            plugin.enabled === true &&
            plugin.version === expectedVersion) === true);
    }
    catch {
        return false;
    }
}
function expectedPluginIsInstalled(runCommand, expectedVersion) {
    const listResult = runCommand?.("codex", ["plugin", "list", "--marketplace", CODEX_MARKETPLACE_NAME, "--json"], { timeoutMs: CODEX_PLUGIN_LIST_TIMEOUT_MS });
