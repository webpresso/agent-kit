...
function isGstackRefreshRequested(env) {
...
function canUseCachedGstack(input) {
    if (!input.hasSetup || !input.hasGitDir || input.refreshRequested)
        return false;
    if (!input.requestsCodex && !input.usesAutoHosts)
        return true;
    if (!input.codexDetected)
        return true;
    return input.hadCodexSkills;
