...
const PRESETS = ['gstack', 'lore-commits', 'omx', 'playwright-mcp', 'rtk', 'vision'];
const DEFAULT_PRESETS = ['omx', 'gstack', 'vision'];
    ...
    try {
        ...
        if (!options.dryRun) {
            runUnifiedSync({
                ...
                check: false,
                allowedSkillSlugs,
                preserveSkillSlugs: new Set(RENDERED_SKILLS),
            });
        }
        ...
        try {
            const pluginJsonPath = join(consumer.repoRoot, 'node_modules', '@webpresso', 'agent-kit', '.claude-plugin', 'plugin.json');
