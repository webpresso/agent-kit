...
import { detectConsumer, isAgentKitTemplateSourceRepo, readPackageJson, setupCommandForRepo, warnIfNonLocalCli, } from './detect-consumer.js';
...
import { disableManagedHooksFromManifest, restoreManagedHooksFromManifest, resolveNodeBinaryForManagedHookLaunchers, scaffoldAgentHooks, trustCodexWebpressoHooksForRepo, trustCodexPresetHooksForUser, } from './scaffolders/agent-hooks/index.js';
...
const USER_VISIBLE_PRESETS = PRESETS.filter((preset) => preset !== 'gstack' && preset !== 'omc' && preset !== 'omx');
...
async function runUserOnlySetup(input) {
    ...
    switch (context7McpResult.action) {
        case 'created':
        case 'overwritten':
            console.log(`  codex context7 mcp: ✓ ${context7McpResult.targetPath} (uses ${CONTEXT7_API_KEY_ENV} from with-secrets)`);
            break;
        case 'identical':
            console.log(`  codex context7 mcp: already configured at ${context7McpResult.targetPath} (launch Codex with: with-secrets -- codex)`);
            break;
        case 'skipped-dry':
    ...
    if (integrations.gstack?.enabled === true) {
        console.log('  gstack: - skipped in user-only mode (run `wp setup` inside a project repo when you need repo-local gstack integration)');
    }
...
async function runHooksRecovery(repoRoot, flags, options) {
    ...
    const nextManifest = restoreHooks
        ...
        : withHookVendorState(manifest, selectedVendors, 'disabled');
    if (!options.dryRun) {
    ...
    if (mutationResult.codex !== undefined) {
        ...
        normalizeGlobalCodexHooksFile(codexHooksPath, {
...
export async function runInit(flags, deps = {}) {
    ...
    try {
        ...
        if (options.dryRun) {
            ...
            writeHooksManifest(consumer.repoRoot, agentHooksResult.manifest.claude, agentHooksResult.manifest.codex, agentHooksResult.manifest.vendorState);
        ...
        if (process.env.WP_SKIP_GSTACK === '1') {
            ...
            switch (gstackResult.kind) {
                case 'gstack-installed':
                    ...
                    switch (gstackResult.codex.kind) {
                        ...
                        case 'gstack-codex-already-configured':
                            console.log(`  gstack (codex): already configured at ${gstackResult.codex.skillsRoot}`);
                            break;
                ...
                case 'gstack-already-configured':
                    console.log(`  gstack: already configured at ${gstackResult.root} (set WP_GSTACK_REFRESH=1 to refresh)`);
                    if (!options.dryRun) {
                        toolingOwnership = claimUserOwnedTool(toolingOwnership, 'gstack');
                        toolingOwnershipChanged = true;
                    }
                ...
                case 'gstack-setup-failed':
                    console.error(gstackResult.reason === 'signal-interrupted'
...
export function registerInitCommand(cli, commandName = 'init') {
    ...
    cli
        ...
        .option('--source-maintenance', "Override the self-repo guard for @webpresso/agent-kit's own setup-surface maintenance (maintainers only)")
        .action(async (flags) => {
