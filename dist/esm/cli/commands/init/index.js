...
import { EXTERNAL_INTEGRATIONS, defaultConfig, mergeConfig, readConfig, writeConfig, } from "./config.js";
import { detectConsumer, readPackageJson, setupCommandForRepo, warnIfNonLocalCli, } from "./detect-consumer.js";
...
import { assertManagedSkillSourcesPresent, isProjectedManagedSkillSlug, scaffoldAgent, SHARED_FAVORITE_SKILLS, } from "./scaffold-agent.js";
...
import { REQUIRED_CORE_CAPABILITIES, auditHostSkillVisibility, parseAgentHosts, serializeHostVisibility, summarizeHostSetupSurfaceVisibility, summarizeHostVisibility, } from "./host-visibility.js";
import { pruneOutdatedAgentKitPluginCaches, summarizePluginCachePrune, } from "./plugin-cache-prune.js";
import { WP_HOOK_SPECS } from "./scaffolders/agent-hooks/ir.js";
import { buildProposedHooksMapFromSpecs, generateHooksDryRunDiff, printSetupReport, } from "./scaffolders/agent-hooks/report.js";
import { readHooksManifest, withHookVendorState, writeHooksManifest, } from "./scaffolders/agent-hooks/manifest.js";
import { buildManagedHooksManifest, disableManagedHooksFromManifest, restoreManagedHooksFromManifest, scaffoldAgentHooks, trustCodexWebpressoHooksForRepo, } from "./scaffolders/agent-hooks/index.js";
...
import { CONTEXT7_API_KEY_ENV, ensureClaudeContext7Mcp, ensureClaudePlaywrightMcp, ensureCodexContext7Mcp, ensureCodexWebpressoMcp, ensureCodexPlaywrightMcp, } from "./scaffolders/codex-mcp/index.js";
...
import { claimProjectOwnedTool, claimUserOwnedTool, clearProjectOwnedTool, readToolingOwnershipState, tryReadRepoKey, writeToolingOwnershipState, } from "#cli/tooling-ownership";
const PRESETS = [
    "example-skill",
    "lore-commits",
    "omc",
...
const DEFAULT_PRESETS = ["vision", "rtk"];
const USER_VISIBLE_PRESETS = PRESETS.filter((preset) => preset !== "omc" && preset !== "omx");
...
function serializeCsvFlag(values) {
    return values.length > 0 ? values.join(",") : undefined;
}
function isPreset(value) {
    return PRESETS.includes(value);
}
function isExternalIntegrationName(value) {
    return EXTERNAL_INTEGRATIONS.includes(value);
}
function resolveIntegrationConfig(existingConfig, flags) {
    void existingConfig;
    const next = {};
    for (const entry of parseCsvFlag(flags.with)) {
        if (!isExternalIntegrationName(entry))
            continue;
        next[entry] = {
            enabled: true,
            ...(entry === "omx" || entry === "omc" ? { scope: "user" } : {}),
        };
    }
    ...
    return Object.fromEntries(EXTERNAL_INTEGRATIONS.flatMap((name) => {
        const config = next[name];
        return config?.enabled ? [[name, config]] : [];
    }));
}
function collectLegacyExternalIntegrationNames(existingConfig, flags) {
    ...
    for (const name of EXTERNAL_INTEGRATIONS) {
        if (existingConfig?.integrations?.[name]?.enabled === true)
            requested.add(name);
    }
    return [...requested].toSorted();
}
function resolveSelectedPresets(flags, integrations) {
    const presets = new Set(DEFAULT_PRESETS);
    for (const explicit of parseCsvFlag(flags.with)) {
        if (isPreset(explicit))
            presets.add(explicit);
    }
    for (const name of EXTERNAL_INTEGRATIONS) {
    ...
    for (const explicitWithout of parseCsvFlag(flags.without)) {
        if (isExternalIntegrationName(explicitWithout))
            presets.delete(explicitWithout);
    }
    return [...presets];
}
function stripPresetValuesFromFlag(value) {
    return serializeCsvFlag(parseCsvFlag(value).filter((entry) => !isPreset(entry)));
}
function normalizeAllToken(rawWith, allFlag) {
...
export function getRepairCommandName(commandName) {
...
function normalizeInitFlagsForExecution(flags) {
...
function formatMovedSetupFlagMessage(commandName, flag) {
    return `wp ${commandName}: \`${flag}\` moved to \`wp ${commandName} repair\`. Re-run \`wp ${commandName} repair ${flag}\`.`;
}
function formatRemovedSetupFlagMessage(commandName, flag) {
    if (flag === "--yes") {
...
function argvHasFlag(args, flag) {
    return args.some((arg) => arg === flag || arg.startsWith(`${flag}=`));
}
export function validatePrimarySetupArgv(commandName, args) {
    if (argvHasFlag(args, "--yes"))
        return formatRemovedSetupFlagMessage(commandName, "--yes");
    if (argvHasFlag(args, "--all"))
        return formatRemovedSetupFlagMessage(commandName, "--all");
    ...
    for (const flag of MOVED_PRIMARY_SETUP_FLAGS) {
        if (argvHasFlag(args, flag))
            return formatMovedSetupFlagMessage(commandName, flag);
    }
    return null;
}
function findInvalidPrimarySetupFlag(flags) {
    if (flags.yes === true)
        return "--yes";
    if (flags.all === true)
        return "--all";
    ...
    if (flags.prune === true)
        return "--prune";
    if ((flags.restoreHooks ?? flags["restore-hooks"] ?? false) === true)
        return "--restore-hooks";
    if ((flags.disableHooks ?? flags["disable-hooks"]) !== undefined)
        return "--disable-hooks";
    if (flags.strict === true)
        return "--strict";
    if ((flags.userOnly ?? flags["user-only"] ?? false) === true)
        return "--user-only";
    if ((flags.projectInit ?? flags["project-init"] ?? false) === true)
        return "--project-init";
    return null;
}
function validatePrimarySetupFlags(commandName, flags) {
...
async function runUserOnlySetup(input) {
    const { repoRoot, packageRoot, options, presets, integrations, selectedHosts, packageVersion, pruneCaches, reason, startMs, } = input;
    ...
    if (reason === "explicit") {
        ...
        console.warn(`wp init: ${repoRoot} does not look like an initialized Webpresso project; falling back to user-only setup.`);
    }
    console.log("  No repo files will be written here. Project scaffolding, repo hooks, and project .mcp.json entries are skipped.");
    ...
    if (presets.includes("playwright-mcp") || presets.includes("omx")) {
        ...
        console.log("  claude playwright mcp: - skipped (project .mcp.json is only written when setup runs inside a project repo)");
    }
    if (isCiEnvironment && presets.includes("omc")) {
        ...
        switch (omcResult.kind) {
            case "omc-installed":
                console.log(`  omc plugin: ✓ user-scope plugin ensured (${omcResult.pluginId}); next in Claude Code: ${OMC_SETUP_COMMAND} --global`);
                break;
    ...
    switch (context7McpResult.action) {
        case "created":
        case "overwritten":
            console.log(`  codex context7 mcp: ✓ ${context7McpResult.targetPath} (uses ${CONTEXT7_API_KEY_ENV} from your secret provider)`);
            break;
        case "identical":
            console.log(`  codex context7 mcp: already configured at ${context7McpResult.targetPath} (launch Codex from a provider-backed shell that exports CONTEXT7_API_KEY)`);
            break;
        case "skipped-dry":
            console.log("  codex context7 mcp: skipped (--dry-run)");
            break;
    }
    console.log("  claude context7 mcp: - skipped (project .mcp.json is only written when setup runs inside a project repo)");
    if (isCiEnvironment) {
        ...
        switch (agentKitGlobalResult.kind) {
            ...
            case "agent-kit-global-repair-failed":
                console.warn(`  agent-kit global: ⚠ root bin/wp launcher repair failed (${agentKitGlobalResult.reason}); ` +
    ...
    if (selectedHosts.includes("codex") && !isCiEnvironment) {
        ...
        switch (codexPluginResult.kind) {
            case "codex-plugin-installed":
                console.log(`  codex plugin: ✓ marketplace + plugin ensured (${codexPluginResult.pluginId}) — restart Codex to load skills`);
                break;
            ...
            case "codex-plugin-timed-out":
                console.warn(`  codex plugin: ⚠ timed out after ${codexPluginResult.timeoutMs}ms while installing ${codexPluginResult.pluginId}; restart Codex and re-run if skills are missing`);
                break;
            case "codex-plugin-failed":
                console.warn(`  codex plugin: ⚠ ${codexPluginResult.step} exited with ${codexPluginResult.exitCode}; ` +
    ...
    if (presets.includes("rtk")) {
        console.log("  rtk: - skipped in user-only mode (RTK setup in this repo also records project-local markers and pins)");
    }
    if (pruneCaches) {
...
function parseDisableHooksTarget(value) {
    if (value === undefined)
        return null;
    if (value === "all")
        return ["claude", "codex"];
...
async function runHooksRecovery(repoRoot, flags, options) {
    ...
    if (manifest === null) {
        console.error(`wp setup: no .webpresso/hooks-manifest.json found — run \`${setupCommandForRepo(repoRoot)}\` first.`);
        return EXIT_SETUP_FAIL;
    }
    const scaffoldInput = { repoRoot, options, trustCodexHooks: false };
    const selectedVendors = disableVendors ?? ["claude", "codex"];
    ...
    const nextManifest = restoreHooks
        ...
        : withHookVendorState(currentManifest, selectedVendors, "disabled");
    ...
    const results = [mutationResult.claude, mutationResult.codex].filter((result) => result !== undefined);
...
function detectPlainDirectoryConsumer(startDir) {
...
export async function runInit(flags, deps = {}) {
    ...
    const hooksRecoveryRequested = (flags.restoreHooks ?? flags["restore-hooks"] ?? false) ||
        (flags.disableHooks ?? flags["disable-hooks"]) !== undefined;
    if (!hasGitRoot && (forceProjectInit || hooksRecoveryRequested)) {
        console.error(`wp setup: ${forceProjectInit ? "--project-init" : "hook recovery"} requires a git working tree (walked up from ${cwd}).\n` +
    ...
    const initializedWebpressoProject = isInitializedWebpressoProject(consumer.repoRoot, consumer.packageJson);
    ...
    if (legacyExternalIntegrations.length > 0) {
        console.log(`  external tools: ${legacyExternalIntegrations.join(", ")} are legacy compatibility presets only; ` +
    ...
    try {
        ...
        if (!options.dryRun) {
            ...
            const activeSkillDirs = new Set(selectUnifiedConsumers(selectedHosts)
        ...
        const persistedHookManifest = previousHooksManifest === null
            ? agentHooksResult.manifest
            : {
                ...agentHooksResult.manifest,
                vendorState: previousHooksManifest.vendorState,
            };
        const disabledHookVendors = previousHooksManifest === null
            ? []
            : ["claude", "codex"].filter((vendor) => previousHooksManifest.vendorState[vendor] === "disabled");
        if (options.dryRun) {
            const proposedMap = buildProposedHooksMapFromSpecs(WP_HOOK_SPECS);
            const dryRunDiff = generateHooksDryRunDiff(join(consumer.repoRoot, ".claude", "settings.json"), join(consumer.repoRoot, ".codex", "hooks.json"), proposedMap, proposedMap);
            stdout.write(dryRunDiff + "\n");
        }
        else {
            if (disabledHookVendors.length > 0) {
                disableManagedHooksFromManifest({ repoRoot: consumer.repoRoot, options, trustCodexHooks: false }, persistedHookManifest, disabledHookVendors);
            }
            writeHooksManifest(consumer.repoRoot, persistedHookManifest.claude, persistedHookManifest.codex, persistedHookManifest.vendorState);
        ...
        switch (claudeContext7McpResult.kind) {
            case "claude-context7-mcp-written":
                console.log(`  claude context7 mcp: ✓ ${claudeContext7McpResult.path} (uses ${CONTEXT7_API_KEY_ENV} from your secret provider)`);
                break;
            case "claude-context7-mcp-unchanged":
                console.log(`  claude context7 mcp: already configured at ${claudeContext7McpResult.path} (launch Claude from a provider-backed shell that exports CONTEXT7_API_KEY)`);
                break;
            ...
            case "claude-context7-mcp-invalid-json":
                console.warn(`  claude context7 mcp: ⚠ ${claudeContext7McpResult.path} is not valid JSON; left unchanged`);
                break;
        }
        ...
        if (selectedHosts.includes("codex") && !isCiEnvironment) {
            ...
            switch (codexPluginResult.kind) {
                ...
                case "codex-plugin-unavailable":
                    ...
                    break;
                case "codex-plugin-timed-out":
                    console.warn(`  codex plugin: ⚠ ${codexPluginResult.step} timed out after ${codexPluginResult.timeoutMs}ms; ` +
        ...
        if (!options.dryRun && repoKey !== null) {
            if (withoutEntries.includes("omx") &&
            ...
            if (withoutEntries.includes("omc") &&
        ...
        if (!options.dryRun) {
            ...
            if (missing.length > 0) {
                if (isCiEnvironment) {
                    ...
                    console.warn(`  host visibility: ⚠ ${missing
                        .map((result) => `${result.host}/${result.capability}`)
...
function registerPrimaryInitOptions(command) {
    const withHelp = `Comma-separated opt-in skills and/or presets to install ` +
...
function registerRepairInitOptions(command) {
    ...
    command
        ...
        .option("--prune", "Remove outdated agent-kit plugin cache versions for supported hosts before visibility checks")
        ...
        .option("--project-init", "Bootstrap repo-local Webpresso project files even when the repo is not initialized yet");
}
export function registerInitCommand(cli, commandName = "init") {
    ...
    registerRepairInitOptions(cli
        .command(getRepairCommandName(commandName), `Advanced repair and maintenance alias for wp ${commandName} repair`)
        .action(async (flags) => {
