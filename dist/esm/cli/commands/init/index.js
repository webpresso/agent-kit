...
import { normalizeGlobalCodexHooksFile, resolveBinaryOnPath, } from './scaffolders/agent-hooks/codex-global-normalize.js';
...
export async function runInit(flags, deps = {}) {
    ...
    try {
        ...
        if (process.env.WP_SKIP_CONTEXT_MODE === '1') {
            ...
            console.log(`  context-mode claude hooks: ${contextModeResult.claudeGlobalHooks.action === 'identical' ? 'already path-stable' : contextModeResult.claudeGlobalHooks.action === 'skipped-dry' ? 'skipped (--dry-run)' : '✓ path-stable'} ${contextModeResult.claudeGlobalHooks.targetPath}`);
        }
        if (isCiEnvironment) {
