...
const REPO_AUDIT_REGISTRY = {
    ...
    "supported-agent-clis": async (root) => (await import("#audit/supported-agent-clis")).auditSupportedAgentClis(root),
    ...
    "no-math-random": async (root) => (await import("#audit/no-math-random")).auditNoMathRandom(root),
    ...
    "atomic-state-writes": async (root) => (await import("#audit/atomic-state-writes")).auditAtomicStateWrites(root),
    ...
    "github-actions-secrets": async (root) => (await import("#audit/github-actions-secrets")).auditGitHubActionsSecrets(root),
    "security-quality-regressions": async (root) => (await import("#audit/security-quality-regressions")).auditSecurityQualityRegressions(root),
...
const AFFECTED_SAFE_AUDIT_KINDS = new Set([
...
export function getAuditScopeSafety(kind) {
    return AFFECTED_SAFE_AUDIT_KINDS.has(kind) ? "affected-safe" : "full-scan-only";
}
export function resolveGuardrailAuditKinds(root, scope = "full") {
    const repoKinds = isAgentKitRoot(root)
        ? REPO_AUDIT_KINDS
        : REPO_AUDIT_KINDS.filter((kind) => kind !== "ai-contracts" && kind !== "absolute-path-policy");
    if (scope === "affected") {
        return repoKinds.filter((kind) => getAuditScopeSafety(kind) === "affected-safe");
    }
    return repoKinds;
}
...
function isAffectedAuditKind(kind) {
    return (kind === "guardrails" || kind === "quality" || getAuditScopeSafety(kind) === "affected-safe");
}
...
export function registerAuditCommand(cli) {
    addAffectedOptions(cli.command("audit [kind] [target]", `Run a packaged audit (${AUDIT_KIND_LIST})`))
        ...
        .option("--changed-only", "Deprecated alias for --affected on supported audits")
        ...
        const affected = Boolean(options.affected ?? options.changedOnly);
        const branch = Boolean(options.branch);
        let dispatchOptions = options;
        let auditScope = "full";
        if (affected || branch) {
            if (kind && AUDIT_KINDS.includes(kind) && !isAffectedAuditKind(kind)) {
                sink.write(`audit ${kind} does not support --affected; run it without --affected.\n`);
                const entry = await sink.finalize({
                    exitCode: 1,
                    summary: `audit ${kind} failed: unsupported affected scope`,
                    options: { kind, target, ...options },
                });
            ...
            const resolution = resolveAffectedTargets({
                ...
                emptyMessage: (scope) => scope === "branch"
                    ? "No affected files changed vs base ref — skipping audit."
                    : "No staged affected files — skipping audit.",
                degradedFallbackMessage: (reason) => `Unable to determine affected files for audit (${reason}); falling back to full audit.`,
                degradedFailClosedMessage: (reason) => `Unable to determine affected files for audit (${reason}); refusing degraded affected audit.`,
            });
            if (resolution.kind === "invalid") {
                sink.write(`${resolution.message}\n`);
                const entry = await sink.finalize({
                    exitCode: 1,
                    summary: "audit failed: invalid affected usage",
                    options: { kind, target, ...options },
                });
                emitCliCommandOutput({
                    entry,
            ...
            if (resolution.kind === "degraded-fail-closed") {
                ...
                const entry = await sink.finalize({
                    exitCode: 1,
                    summary: "audit failed: affected resolution degraded",
                    options: { kind, target, ...options },
                });
                emitCliCommandOutput({
                    entry,
            ...
            if (resolution.kind === "empty") {
                ...
                const entry = await sink.finalize({
                    exitCode: 0,
                    summary: kind ? `audit ${kind} skipped: no affected files` : "audit skipped",
                    options: { kind, target, ...options },
                });
                emitCliCommandOutput({
                    entry,
        ...
        const outcome = await runAuditDispatch(kind, target ? [target] : [], dispatchOptions, {
            root: process.cwd(),
            ...
            knownRepoKinds: kind === "guardrails" || kind === "quality"
                ? resolveGuardrailAuditKinds(auditRoot, auditScope)
                : REPO_AUDIT_KINDS,
        });
        let exitCode = 1;
        ...
        switch (outcome.kind) {
            ...
            case "repo-result": {
                ...
                sink.write(await formatAuditResult(outcome.result, dispatchOptions));
                summary =
                    kind === undefined
                        ? exitCode === 0
                            ? "audit passed"
