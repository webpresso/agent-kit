    ...
    cli
        ...
        const outcome = await runAuditDispatch(kind, target ? [target] : [], options, {
            ...
            runScript: async (script, args) => {
                const result = await runLoggedChildCommand({ command: 'bun', args: [script, ...args] }, { write: (chunk) => sink.write(chunk) });
                if (result.exitCode !== 0 && readTrailingFileIsEmpty(sink.absoluteLogPath)) {
                    sink.write('Failed to spawn audit runner (bun). Install Bun (https://bun.sh) or run the audit script directly.\n');
                }
                return result.exitCode;
            },
...
function readTrailingFileIsEmpty(pathToFile) {
    try {
