...
export function registerPreviewCommand(cli) {
    cli
        ...
        .action((flags) => runPreviewCommand({
        cwd: process.cwd(),
        lane: flags.lane,
...
export async function runPreviewCommand(options = {}, deps = {}) {
    try {
        ...
        if (options.json)
            writer.write(`${JSON.stringify(toWpErrorJson(wpError), null, 2)}\n`);
        else
            writer.write(`${formatWpError(wpError)}\n`);
        return 1;
    }
}
