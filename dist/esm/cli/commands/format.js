    ...
    cli
        ...
        if (affected) {
            ...
            if (selection.degraded) {
                if (!check) {
                    console.error(`Unable to determine affected files for format (${selection.reason}); refusing a degraded whole-repo write. Rerun without --affected, pass --check, or target files explicitly.`);
                    return 1;
                }
                console.error(`Unable to determine affected files for format --check (${selection.reason}); falling back to a whole-repo format check.`);
