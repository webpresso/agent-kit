    ...
    addAffectedOptions(cli
        ...
        if (affected || branch) {
            const resolution = resolveAffectedTargets({
                ...
                degradedFallbackMessage: (reason) => `Unable to determine affected files for typecheck (${reason}); falling back to the full typecheck surface.`,
                degradedFailClosedMessage: (reason) => `Unable to determine affected files for typecheck (${reason}); refusing degraded affected execution.`,
            }, deps);
