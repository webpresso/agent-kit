...
import { readToolingOwnershipState, tryReadRepoKey, writeToolingOwnershipState, } from "#cli/tooling-ownership";
...
import { claimOptionalToolOwnership, clearOptionalToolOwnership, formatOptionalToolInstallSuccess, formatOptionalToolRemoveSuccess, optionalToolUpdateSteps, parseOptionalToolCommandArgs, spawnResultStatus, } from "#cli/optional-tools.js";
export const PACKAGE_MANAGER_VERBS = ["install", "add", "remove", "update", "exec", "run"];
const HELP_BY_VERB = {
    install: "Install dependencies through the managed package/task facade, or WP-managed agent tools such as codex, claude-code, opencode, and oh-my tools.",
    add: "Add dependencies through the managed package/task facade.",
    remove: "Remove dependencies through the managed package/task facade, or clear WP ownership for managed agent tools without native uninstall.",
    update: "Refresh wp and any WP-owned optional agent tools; use --deps for local dependency updates through the managed package/task facade.",
...
function runOptionalToolPackageCommand(verb, deps) {
    const argv = deps.argv ?? process.argv;
    ...
    if (parsed.kind === "error")
        return failUsage(`wp ${verb}: ${parsed.message}`);
    const cwd = deps.cwd ?? process.cwd();
    ...
    if (verb === "remove") {
        const nextState = clearOptionalToolOwnership(ownershipState, parsed.adapter, parsed.scope, repoKey);
        if ("error" in nextState)
            return failUsage(`wp remove: ${nextState.error}`);
    ...
    const ownership = claimOptionalToolOwnership(ownershipState, parsed.adapter, parsed.scope, repoKey);
    if ("error" in ownership)
        return failUsage(`wp install: ${ownership.error}`);
...
function runOptionalToolStep(step, deps) {
