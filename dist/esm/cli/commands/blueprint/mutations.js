...
async function promoteBlueprintLocked(cwd, slug, toState) {
    ...
    if (currentState === "draft" && toState === "planned") {
        ...
        const distinctApprovals = countDistinctLogBackedApprovals(currentDocumentPath, matter(content).data.approvals);
        if (distinctApprovals < 2) {
            throw new Error(`Cannot promote "${slug}" to planned: ${distinctApprovals} distinct reviewer approval(s) in frontmatter \`approvals:\` are backed by committed review evidence (need ≥2). Record approvals from distinct reviewers and commit the matching review ledger (e.g. reviews.md) — see catalog/agent/rules/pre-implementation.md.`);
        }
    }
