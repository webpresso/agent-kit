...
async function promoteBlueprintLocked(cwd, slug, toState) {
    ...
    if (currentState === "draft" && toState === "planned") {
        ...
        const distinctApprovals = countDistinctApprovals(matter(content).data.approvals);
        if (distinctApprovals < 2) {
            throw new Error(`Cannot promote "${slug}" to planned: ${distinctApprovals} distinct reviewer approval(s) in frontmatter \`approvals:\` (need ≥2). Record approvals from distinct reviewers (e.g. /plan-eng-review, /codex, /deepseek) — see catalog/agent/rules/pre-implementation.md.`);
        }
    }
