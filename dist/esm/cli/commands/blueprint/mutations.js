...
import { countDistinctProvenanceBackedApprovals } from "#lifecycle/audit";
/**
...
function readPlatformMutationTimeoutMs() {
    ...
    if (raw === undefined)
        return DEFAULT_PLATFORM_MUTATION_TIMEOUT_MS;
    const parsed = Number.parseInt(raw, 10);
    return Number.isFinite(parsed) && parsed > 0 ? parsed : DEFAULT_PLATFORM_MUTATION_TIMEOUT_MS;
}
async function awaitPlatformMutationStep(promise, label, step, timeoutMs) {
    let timeout;
    try {
        return await Promise.race([
            promise,
            new Promise((_, reject) => {
                timeout = setTimeout(() => reject(new Error(`${step} timed out after ${timeoutMs}ms`)), timeoutMs);
            }),
        ]);
    }
    catch (error) {
...
async function runCliPlatformMutationSync(adapter, options) {
    if (adapter === null)
        return;
...
async function promoteBlueprintLocked(cwd, slug, toState) {
    ...
    if (currentState === "draft" && toState === "planned") {
        ...
        const distinctApprovals = countDistinctProvenanceBackedApprovals(currentDocumentPath, matter(content).data.approvals);
        if (distinctApprovals < 2) {
            throw new Error(`Cannot promote "${slug}" to planned: ${distinctApprovals} distinct provenance-backed reviewer approval(s) in frontmatter \`approvals:\` are backed by committed review evidence artifacts (need ≥2). Record approvals from distinct reviewers, commit the matching review ledger (e.g. reviews.md), and include tracked outside-model review artifacts referenced by each structured review entry — see catalog/agent/rules/pre-implementation.md.`);
        }
    }
