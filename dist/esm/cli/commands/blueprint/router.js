...
let blueprintExecCommandExists = defaultCommandExists;
let blueprintExecStarter = null;
...
function assertBlueprintExecutionPrereqs() {
    if (blueprintExecCommandExists("omx"))
        return;
    throw new Error([
...
export async function moveBlueprint(slug, status, options = {}) {
    ...
    if (location.blueprint.status === "draft" && nextStatus === "planned") {
        throw new Error("Blueprint move cannot recover a draft directly into planned. Use `wp blueprint promote <slug> planned` so the Trust Dossier gate runs.");
    }
...
export function _setBlueprintExecCommandExistsForTests(probe) {
    blueprintExecCommandExists = probe ?? defaultCommandExists;
}
export function _setBlueprintExecStarterForTests(starter) {
