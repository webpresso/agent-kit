...
import { createE2eExecutionPlan, formatShellCommand, plannedGroupsToCommandConfigs, } from '#e2e/execution';
...
export function registerE2eCommand(cli) {
    cli
        ...
        const result = await runCliCommandSequence({
            ...
            commands,
            cwd: process.cwd(),
            timeoutMs: flags.timeoutMs === undefined ? undefined : Number.parseInt(String(flags.timeoutMs), 10),
            metadataOptions: {
                suite: flags.suite,
                runner: flags.runner,
                file: files,
