...
function normalizeWpArgs(args) {
    return args[0] === "--" ? args.slice(1) : args;
}
export function stripLeadingSecretWrappers(command) {
    let next = command.trim();
    while (next) {
        const tokens = tokenizeCommand(next);
        const first = tokens[0];
        if (!first || !SECRET_WRAPPER_BINS.has(first))
            return next;
        let index = 1;
        if ((first === "doppler" || first === "infisical") && tokens[index] === "run")
            index += 1;
        if (tokens[index] === "--")
            index += 1;
...
function isWrappedWpShorthand(manager, subcommand) {
    if (manager === "pnpm")
        return subcommand === "wp" && !PNPM_BUILTIN_SUBCOMMANDS.has(subcommand);
    if (manager === "yarn" || manager === "yarnpkg") {
        return subcommand === "wp" && !YARN_BUILTIN_SUBCOMMANDS.has(subcommand);
    }
    if (manager === "bun")
        return subcommand === "wp" && !BUN_BUILTIN_SUBCOMMANDS.has(subcommand);
    if (manager === "vp")
        return subcommand === "wp";
    return false;
}
export function detectWrappedWpCommand(command) {
    const trimmed = stripLeadingSecretWrappers(stripCorepackPackageManagerProxy(stripLeadingEnvironmentAssignments(command)));
    ...
    if (!isWrappedWpShorthand(manager, subcommand))
        return null;
    return { manager, wpArgs: normalizeWpArgs(tokens.slice(index + 1)) };
}
function detectPackageManagerFromEnv(env) {
    ...
    if (normalized.includes("npm"))
        return "npm";
    if (env.VP_COMMAND === "run" || env.VP_CLI_BIN)
        return "vp";
    return null;
}
function defaultReadProcessInfo(pid) {
    try {
        ...
        if (!match?.groups?.ppid || !match.groups.command)
            return null;
...
export function detectWrappedWpRuntimeInvocation(options = {}) {
    ...
    for (let depth = 0; depth < maxDepth && pid > 1 && !seen.has(pid); depth += 1) {
        ...
        if (wrapped)
            return { manager: wrapped.manager, wpArgs: normalizeWpArgs(argv.slice(2)) };
        pid = info.ppid;
    }
    return null;
}
function firstWpVerb(wpArgs) {
...
export function formatWrappedWpInvocationError(wrapped, argv) {
...
//# sourceMappingURL=wrapped-wp.js.map