...
const ROOT_HELP_CORE_ENTRIES = [
    ...
    {
        command: "install",
        description: "Install dependencies, or WP-managed agent tools (codex, claude-code, opencode, oh-my ...)",
    },
    ...
    {
        command: "update",
        description: "Refresh wp and WP-owned optional agent tools by default; use --deps for local dependencies (--global is an alias)",
    },
...
const ROOT_HELP_QUALITY_ENTRIES = [
    {
        command: "audit",
...
const ROOT_HELP_ADVANCED_ENTRIES = [
...
function renderRootHelpEntries(entries) {
    return entries.map(({ command, description }) => `  ${command.padEnd(21, " ")}${description}`);
}
function buildRootHelp(options = {}) {
    const lines = [
    ...
    lines.push("", "Options:", "  -h, --help     Display this message", "  -v, --version  Display version number", "", "Run `wp <command> --help` for command-specific help.");
    return lines.join("\n");
}
...
export async function main() {
    ...
    const repairSubcommand = (command === "setup" || command === "init") && argv[3] === "repair" ? command : null;
    ...
    cli.version(VERSION);
    try {
        const effectiveArgv = repairSubcommand !== null
