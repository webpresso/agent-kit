...
import { getCommandRawArgs, placeholderCommand, placeholderGroup, } from "#cli/bundle/commands/helpers.js";
const SCOPE = "Blueprint";
function hasFlag(args, flag) {
    return args.includes(flag);
}
function createBlueprintAuditCommand() {
    return {
        ...
        run: async (context) => {
            ...
            if (hasFlag(rawArgs, "--all"))
                args.push("--all");
            if (hasFlag(rawArgs, "--staged"))
                args.push("--staged");
            if (hasFlag(rawArgs, "--strict"))
                args.push("--strict");
            if (asJson)
                args.push("--json");
