...
export const rootContractMode = "js-selector-runtime-lane";
export const expectedRootWpBinRelativePath = "bin/wp";
export const rootWpSelectorSource = "#!/usr/bin/env node\n\nimport { runNamedBin } from './_run.js'\n\nrunNamedBin('wp')\n";
function normalizePath(path) {
    return path.replaceAll("\\", "/");
}
function normalizeLauncherSource(text) {
    return text.replaceAll("\r\n", "\n");
}
function isExecutableMode(mode) {
    return process.platform === "win32" || (mode & 0o111) !== 0;
}
function isRuntimeTargetSymlinkTarget(targetPath) {
    return /(^|\/)(?:bin\/)?runtime\/[^/]+\/wp(?:\.exe)?$/u.test(normalizePath(targetPath));
}
function isJavaScriptSelector(source) {
...
export function validateRootLauncherContract(path) {
    let stat;
    try {
        stat = lstatSync(path);
    }
    ...
    if (!stat.isFile())
        return { ok: false, code: "not-file", path };
    if (!isExecutableMode(stat.mode))
        return { ok: false, code: "not-executable", path };
    let source;
    try {
...
export function formatRootLauncherContractSuccess(subject = expectedRootWpBinRelativePath) {
    return `${subject} satisfies the ${rootContractMode} root launcher contract`;
}
export function formatRootLauncherContractFailure(result, subject = expectedRootWpBinRelativePath) {
    switch (result.code) {
        case "missing":
...
//# sourceMappingURL=root-contract.js.map