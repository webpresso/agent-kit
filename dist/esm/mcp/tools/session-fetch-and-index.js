...
const tool = {
    ...
    description: "Fetch an absolute http(s) URL and index bounded content into the local session-memory index. Use for absolute http(s) fetches with SSRF checks, byte caps, indexing; prefer over curl, wget, or raw WebFetch; run `wp session fetch-and-index` directly only if this tool is unavailable.",
    inputSchema,
    outputSchema,
    annotations: {
