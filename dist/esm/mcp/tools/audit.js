...
const tool = {
    name: 'ak_audit',
    description: 'Run a packaged repo audit. `kind` selects the audit (tph, tph-e2e, catalog-drift, docs-frontmatter, blueprint-lifecycle, bundle-budget, commit-message, tech-debt). Returns {passed, kind, details}.',
    inputSchema,
    outputSchema,
    annotations: {
        title: 'Audit',
    ...
    handler: async (raw) => {
        ...
        try {
            ...
            return wrap({ passed: false, summary: `Invalid ak_audit input for ${kind}`, kind, details: message }, { isError: true });
        }
        try {
            ...
            const message = error instanceof Error ? error.message : String(error);
            return wrap({ passed: false, summary: `${input.kind} audit crashed`, kind: input.kind, details: message }, { isError: true });
        }
    },
};
export default tool;
