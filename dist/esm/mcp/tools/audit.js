...
export function wrapAuditPayload(payload, options = {}) {
    return createSummaryResult(payload, options);
}
export function summarizeRepoAudit(kind, result) {
...
export async function dispatchAudit(input) {
    const { kind } = input;
    switch (kind) {
        case "catalog-drift": {
        ...
        case "reference-parity-matrix": {
            ...
            const auditResult = auditReferenceParityMatrix(input.cwd ?? input.directory ?? process.cwd(), undefined, { requireReleaseReady: input.strict });
            return {
                passed: auditResult.ok,
                summary: summarizeRepoAudit(kind, auditResult),
                kind,
        ...
        case "blueprint-lifecycle": {
            ...
            const auditResult = await auditBlueprintLifecycleSql(input.cwd ?? input.directory ?? process.cwd(), { baseRef: input.baseRef });
            return {
                passed: auditResult.ok,
                summary: summarizeRepoAudit(kind, auditResult),
                kind,
        ...
        case "security-quality-regressions": {
            const { auditSecurityQualityRegressions } = await import("#audit/security-quality-regressions");
            const auditResult = auditSecurityQualityRegressions(input.cwd ?? input.directory ?? process.cwd());
            return {
                passed: auditResult.ok,
                summary: summarizeRepoAudit(kind, auditResult),
                kind,
        ...
        case "consumer-agent-kit-dependency": {
            const { auditConsumerAgentKitDependency } = await import("#audit/consumer-agent-kit-dependency");
            const auditResult = auditConsumerAgentKitDependency(input.cwd ?? input.directory ?? process.cwd());
            return {
                passed: auditResult.ok,
                summary: summarizeRepoAudit(kind, auditResult),
                kind,
...
const tool = {
    ...
    handler: async (raw) => {
        ...
        try {
            ...
            return wrapAuditPayload({ passed: false, summary: `Invalid wp_audit input for ${kind}`, kind, details: message }, { isError: true });
        }
        try {
