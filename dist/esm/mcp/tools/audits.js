...
import { MCP_AUDIT_KINDS, isMCPAuditKind } from './_shared/audit-kinds.js';
import { createSummaryOutputSchema, createSummaryResult } from './_shared/result.js';
import { dispatchAudit, KINDS } from './audit.js';
export function resolveGuardrailPresetKinds(root = process.cwd()) {
    return resolveGuardrailAuditKinds(root).filter(isMCPAuditKind);
}
const ALL_PRESET = MCP_AUDIT_KINDS.filter((kind) => kind !== 'commit-message' && kind !== 'bundle-budget');
const inputSchema = z
    .object({
...
const tool = {
    name: 'wp_audits',
    description: 'Run multiple packaged repo audits in one deterministic batch. Provide `kinds` or preset `guardrails`/`all`; returns aggregate pass/fail plus per-audit results.',
    inputSchema,
    outputSchema,
    annotations: {
        title: 'Batch audits',
