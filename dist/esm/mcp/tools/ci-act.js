...
const inputSchema = z
    ...
    envProfile: z
        ...
        message: "envProfile is a secret-gate runtime profile (none, public, secrets-only, service-runtime, database, full). Use secretProfile for repo-owned secret selectors.",
    }),
...
const tool = {
    name: "wp_ci_act",
    description: "Run local GitHub Actions workflows through `act` via the public secret contract (`wp secrets doctor --profile <profile> --json`, then `wp secrets run --sink act --profile <profile> -- act ...`). Use for running act / local GitHub Actions; prefer over act or vp exec act; run `wp ci act` directly only if this tool is unavailable.",
    inputSchema,
    outputSchema,
    annotations: {
        title: "CI act",
    ...
    handler: async (raw, extra) => {
        ...
        try {
            ...
            const details = prepared.mode === "replay"
                ? {
                    command: publicCommandDetails(input, cwd),
                    envProfile: input.envProfile,
                    mode: "replay",
