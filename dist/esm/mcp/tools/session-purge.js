...
function hasScope(input) {
    return Boolean(input.repoHash || input.sessionId || input.source);
}
const tool = {
    name: "wp_session_purge",
    description: "Dry-run or explicitly confirm scoped local session-memory purge operations. Use for an explicit session-memory reset (dry-run first; confirm deletion); prefer over raw rm of the store; run `wp session purge` directly only if this tool is unavailable.",
    inputSchema,
    outputSchema,
    annotations: {
        title: "Session purge",
    ...
    handler: async (raw) => {
        ...
        try {
            const sessionResult = input.target === "indexed_chunks"
                ? {
            ...
            const shouldPurgeIndex = input.target !== "continuity_events" &&
            ...
            const deletedTotal = sessionResult.deletedEventCount +
                sessionResult.deletedSnapshotCount +
                indexResult.deletedCount;
            const matchedTotal = sessionResult.matchedEventCount +
