...
import { commandCounts, readonlyOpsBaseSchema, resolveReadonlyCwd, runReadonlyCommand, } from "./_readonly-ops.js";
...
function prViewFields(includeChecks, includeReviews) {
    return [
        "number",
        "title",
        "state",
...
const tool = {
    name: "wp_pr_status",
    description: "Read-only PR status summary via GitHub CLI: branch PR metadata, optional checks, and optional reviews with bounded output. Use for PR status / GitHub checks / review decision; prefer over gh pr view or gh pr checks; run `wp pr status` directly only if this tool is unavailable.",
    inputSchema,
    outputSchema,
    annotations: {
        title: "PR status",
    ...
    handler: async (raw, extra) => {
        ...
        if (input.includeChecks) {
            commands.push(await runReadonlyCommand("pr_status_checks", "gh", ["pr", "checks", ...selector, "--json", "name,state,bucket,link,description"], {
                cwd,
                timeoutMs: input.timeoutMs,
                maxOutputBytes: input.maxOutputBytes,
                signal: extra?.signal,
