...
const tool = {
    name: "wp_lint",
    description: "Run lint via the `vp lint` facade. Returns `{passed, issues: [{file, line, rule, message}]}`. Use for linting / code-style checks; prefer over oxlint or bun run lint; run `wp lint` directly only if this tool is unavailable.",
    inputSchema,
    outputSchema,
    annotations: {
        title: "Lint",
