...
import { createSummaryOutputSchema, createSummaryResult, elisionSchema, failureSchema, } from "./_shared/result.js";
...
function toCompactLeaf(result, options = {}) {
    ...
    return {
        ...
        ...(Array.isArray(result.warnings)
            ? {
                warnings: result.warnings.filter((warning) => typeof warning === "string"),
            }
            : {}),
...
const UI_QA_HINT = "Static QA passed. For visual/UX QA, run /qa or /qa-only with the Webpresso browser skills.";
function withDefinedValues(input) {
    return Object.fromEntries(Object.entries(input).filter(([, value]) => value !== undefined && value !== false));
}
...
const tool = {
    name: "wp_qa",
    description: "Run `wp_lint`, `wp_typecheck`, and `wp_test` in parallel via `Promise.all`. Returns `{passed, lint, typecheck, test}` where the top-level `passed` is the AND of the three sub-results. Use for full QA pass / markdown lint; prefer over bun run qa or markdownlint-cli2; run `wp qa` directly only if this tool is unavailable.",
    inputSchema,
    outputSchema,
    annotations: {
    ...
    handler: async (raw, extra) => {
        ...
        const [lintResult, typecheckResult, testResult] = await Promise.all([
            lintTool.handler(withDefinedValues({ cwd: input.cwd, files: input.files, full: input.full }), extra),
            typecheckTool.handler(withDefinedValues({ cwd: input.cwd, packages: input.packages, full: input.full }), extra),
            testTool.handler(withDefinedValues({
                cwd: input.cwd,
                files: input.files,
