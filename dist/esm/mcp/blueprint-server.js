...
import { BpDetailRowSchema, BpRowSchema, TaskRowCompactSchema, TaskRowSchema, } from "#db/types.js";
...
import { resolveSyncAdapter, runPlatformMutationSync, _setSyncAdapterFactory, } from "#mcp/blueprint/_shared/sync";
...
function parseProjectionRows(schema, rows, failures, label) {
    const parsed = [];
...
function parseProjectionRow(schema, row, failures, label) {
...
async function handleTaskNext(projectResolver, cwd, raw) {
    ...
    try {
        ...
        try {
            task =
                parseProjectionRows(TaskRowSchema, blueprint ? conn.db.prepare(readySql).all(blueprint) : conn.db.prepare(readySql).all(), failures, "ready tasks projection")[0] ?? null;
            w0cnt =
                (blueprint
...
async function handlePromote(projectResolver, cwd, raw) {
    ...
    const trustedSource = trustedMarkdown === undefined ? undefined : readFileSync(overviewPath, "utf8");
...
function createListBpReader(failures) {
    return ({ db, project }) => {
        ...
        return parseProjectionRows(BpRowSchema, db.prepare(sql).all(), failures, `blueprints projection ${project.project_id}`);
    };
}
...
function listCurrentProjectBlueprintRows(cwd, options, failures = []) {
    const target = dbPath(cwd);
    if (!existsSync(target))
        return [];
    const conn = openDb(target);
    try {
        ...
        return parseProjectionRows(BpRowSchema, options.status
            ? conn.db.prepare(sql).all(options.status, options.limit)
            : conn.db.prepare(sql).all(options.limit), failures, "blueprints projection");
    }
    finally {
        conn.close();
    }
}
function getCurrentProjectBlueprint(cwd, slug, failures = []) {
    const target = dbPath(cwd);
    if (!existsSync(target))
        return { blueprint: null, tasks: [] };
    const conn = openDb(target);
    try {
        const blueprint = parseProjectionRow(BpDetailRowSchema, conn.db
            ...
            .get(slug) ?? null, failures, `blueprint ${slug}`);
        const tasks = blueprint
            ? parseProjectionRows(TaskRowCompactSchema, conn.db
                .prepare(`SELECT task_id, title, status, wave, lane FROM tasks WHERE blueprint_slug = ? ORDER BY id`)
                .all(slug), failures, `tasks for ${slug}`)
            : [];
    ...
    try {
        ...
        try {
            const sql = status
                ? `SELECT slug, title, status, complexity, owner, last_updated, content_hash, ingested_at, file_path FROM blueprints WHERE status = ? ORDER BY ingested_at DESC LIMIT ?`
                : `SELECT slug, title, status, complexity, owner, last_updated, content_hash, ingested_at, file_path FROM blueprints ORDER BY ingested_at DESC LIMIT ?`;
            rows = parseProjectionRows(BpRowSchema, status ? conn.db.prepare(sql).all(status, limit) : conn.db.prepare(sql).all(limit), failures, "blueprints projection");
        }
        finally {
            conn.close();
        }
...
async function handleBlueprintGet(projectResolver, cwd, raw) {
    ...
    if (isMultiScope) {
        try {
            ...
            const timed = await awaitBounded(aggregateBlueprintRows({
                target: readTarget,
                read: ({ db, project }) => {
                    return parseProjectionRows(BpDetailRowSchema, db
                        .prepare(`SELECT slug, title, status, complexity, owner, last_updated, content_hash, ingested_at, file_path FROM blueprints WHERE slug = ?`)
                        .all(slug), rowFailures, `blueprints projection ${project.project_id}`);
                },
                resolveOptions: { cwd },
            }), readBoundedTimeoutMs("WP_BLUEPRINT_PROJECT_DISCOVERY_TIMEOUT_MS", DEFAULT_PROJECT_DISCOVERY_TIMEOUT_MS));
            if (timed.timedOut) {
            ...
            if (owningProject) {
                ...
                if (existsSync(owningDbPath)) {
                    ...
                    try {
                        tasks = parseProjectionRows(TaskRowCompactSchema, conn.db
                            .prepare(`SELECT task_id, title, status, wave, lane FROM tasks WHERE blueprint_slug = ? ORDER BY id`)
                            .all(slug), rowFailures, `tasks for ${slug}`);
                    }
                    finally {
                        conn.close();
                    }
    ...
    try {
        ...
        let tasks;
        try {
            blueprint = parseProjectionRow(BpDetailRowSchema, conn.db
                .prepare(`SELECT slug, title, status, complexity, owner, last_updated, content_hash, ingested_at, file_path FROM blueprints WHERE slug = ?`)
                .get(slug) ?? null, failures, `blueprint ${slug}`);
            tasks = blueprint
                ? parseProjectionRows(TaskRowCompactSchema, conn.db
...
function renderBlueprintMarkdownFromDocument(slug, document) {
    ...
    const trustDossierBlock = document.trust_dossier
        ...
        : [];
    return ([...frontmatter, ...sections, ...taskBlocks, ...trustDossierBlock].join("\n").trimEnd() + "\n");
}
function renderTrustDossierMarkdown(dossier) {
    ...
    return [
        ...
        "### Material Claims",
        "",
        ...renderMarkdownTable(["ID", "Claim", "Evidence"], dossier.material_claims.map((claim) => [claim.id, claim.claim, claim.evidence])),
        "",
        "### Material Decisions",
        "",
        ...renderMarkdownTable(["ID", "Decision", "Chosen option", "Rejected alternatives", "Rationale"], dossier.material_decisions.map((decision) => [
            decision.id,
            decision.decision,
...
async function handleBlueprintPut(projectResolver, cwd, raw) {
    ...
    if (document.status !== "draft") {
        return err("wp_blueprint_put refused", `wp_blueprint_put only writes draft blueprints; use wp_blueprint_transition for executable lifecycle states.`);
    }
...
async function applyLocalBlueprintTransition(input) {
    ...
    const trustedMarkdown = preflightTrustedMarkdown ??
...
export async function registerBlueprintTools(registrar, cwd, projectResolver = createProjectResolver()) {
    ...
    registrar.registerTool("wp_blueprint_put", "Create or replace a blueprint from a structured whole-document payload. Renders markdown, validates it, re-ingests the projection, and returns revision metadata.", putInputJsonSchema, {
...
async function handleProjects(projectResolver, cwd, ensureRoots, raw) {
    ...
    const projects = timedProjects.timedOut
        ? [buildFallbackCurrentProject(cwd)]
        : timedProjects.value;
