...
export function validateFile(file) {
    ...
    if (!existsSync(file)) {
        console.warn(`⚠️  check-refs: skipping unreadable path (broken symlink): ${relative(DOCS_ROOT, file)}`);
        return dead;
    }
    ...
    try {
        ...
        console.warn(`⚠️  check-refs: skipping unreadable path: ${relative(DOCS_ROOT, file)} (${reason})`);
        return dead;
    }
