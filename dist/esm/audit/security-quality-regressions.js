...
const TEXT_RULES = [
    ...
    },
    {
        ...
        message: "use shared markdown table cell escaping or HTML entities instead of pipe-only backslash escaping",
        isViolation: (line) => line.includes(".replace(/\\|/g") && (line.includes('"\\\\|"') || line.includes("'\\\\|'")),
    },
];
function toRel(root, file) {
    return relative(root, file).replace(/\\/gu, "/");
}
function isWorkflowFile(root, file) {
    const rel = toRel(root, file);
    return (rel.startsWith(`${WORKFLOW_ROOT_PARTS[0]}/${WORKFLOW_ROOT_PARTS[1]}/`) &&
...
function walkFiles(root, dir, files) {
    ...
    for (const entry of readdirSync(dir, { withFileTypes: true })) {
        ...
        if (!entry.isFile())
            continue;
        if (isSourceFile(fullPath) || isWorkflowFile(root, fullPath))
            files.push(fullPath);
    }
}
function hasTopLevelWorkflowPermissions(content) {
    return content.split(/\r?\n/u).some((line) => /^permissions:\s*(?:$|\S)/u.test(line));
}
function allWorkflowJobsHavePermissions(content) {
...
function checkWorkflowPermissions(root, file) {
    const content = readFileSync(file, "utf8");
    if (hasTopLevelWorkflowPermissions(content) || allWorkflowJobsHavePermissions(content))
        return [];
    const rel = toRel(root, file);
    return [
        {
...
function checkSourceRules(root, file) {
    const rel = toRel(root, file);
    const violations = [];
    const lines = readFileSync(file, "utf8").split(/\r?\n/u);
    for (const [index, line] of lines.entries()) {
        for (const rule of TEXT_RULES) {
            if (!rule.isViolation(line))
                continue;
            violations.push({
                file: rel,
...
export function auditSecurityQualityRegressions(rootDirectory = process.cwd()) {
    const files = [];
    walkFiles(rootDirectory, rootDirectory, files);
    const violations = files.flatMap((file) => isWorkflowFile(rootDirectory, file)
...
//# sourceMappingURL=security-quality-regressions.js.map