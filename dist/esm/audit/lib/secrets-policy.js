...
export function resolveSecretsAuditRoot(rootDirectory = process.cwd()) {
    const absoluteRoot = resolve(rootDirectory);
    const gitRoot = resolveGitTopLevel(absoluteRoot);
    if (gitRoot && existsSync(join(gitRoot, SECRETS_CONFIG_PATH)))
        return gitRoot;
    let current = absoluteRoot;
    while (true) {
        if (existsSync(join(current, SECRETS_CONFIG_PATH)))
            return current;
...
function validateConfigKeys(obj, sourceLabel, allowedKeys = ALLOWED_CONFIG_KEYS) {
...
function requireRecord(value, sourceLabel, name) {
    if (typeof value !== 'object' || value === null || Array.isArray(value)) {
        throw new Error(`${sourceLabel}: "${name}" must be an object`);
    }
    return value;
}
function buildProfilesMetadata(value, sourceLabel, allowedProviders) {
    if (value === undefined)
        return undefined;
...
function buildSchemaVersion1Metadata(obj, sourceLabel) {
    ...
    const base = profiles ? { manager, projectId: project, profiles } : { manager, projectId: project };
    if (obj.projectLabel === undefined)
        return base;
    if (typeof obj.projectLabel !== 'string' || obj.projectLabel.length === 0) {
        throw new Error(`${sourceLabel}: "projectLabel" must be a non-empty string when set`);
...
export function parseSecretsConfigMetadata(raw, sourceLabel) {
    ...
    if (schemaVersion1Metadata)
        return schemaVersion1Metadata;
