...
const DEFAULT_PUBLISHED_PACKAGE_BASELINES = {
...
export function auditPackageSurface(rootDirectory = process.cwd(), options = {}) {
    ...
    const publishedBaselines = contract.publishedPackageBaselines ?? DEFAULT_PUBLISHED_PACKAGE_BASELINES;
...
function auditConsumerAgentKitDependencyPolicy(root, violations) {
    let checked = 0;
    for (const packageFile of walkFiles(root, (file) => basename(file) === "package.json")) {
        ...
        for (const [fieldName, deps] of dependencyFields) {
            if (!deps || typeof deps !== "object")
                continue;
            checked += 1;
            if (!("@webpresso/agent-kit" in deps))
                continue;
            violations.push({
                file: relPath,
...
export function parseNpmPackJsonOutput(raw) {
    try {
...
function defaultSecretlintConfig(toolRoot) {
...
function auditPublishedPackageBaseline(root, packageFile, pkg, baselines, violations) {
