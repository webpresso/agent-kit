...
function checkHookFile(root, relativePath, host, violations) {
    ...
    for (const requirement of REQUIRED_HOOKS) {
        ...
        if (!found) {
            const supportDetail = requirement.event === 'PreCompact'
...
function checkClaudeWorktree(root, violations) {
    ...
    if (!existsSync(settingsPath))
        return;
    const parsed = readTrustedJsonFile(settingsPath);
...
function checkAgentKitDevDependency(root, packageJson, violations) {
    ...
    if (typeof version !== 'string' || version.trim().length === 0) {
        violations.push({
            file: 'package.json',
            message: 'Missing devDependency `@webpresso/agent-kit`. Pin a published semver range, run `vp install`, then use global `wp setup`.',
        });
    }
}
