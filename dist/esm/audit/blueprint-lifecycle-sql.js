...
function repoRelativePathForGit(cwd, filePath) {
    ...
    if (blueprintsIdx >= 0)
        return segments.slice(blueprintsIdx).join("/");
    return normalized;
}
...
function blueprintSlugFromGitPath(filePath) {
    const segments = normalizeGitPath(filePath).split("/").filter(Boolean);
    const blueprintsIdx = segments.lastIndexOf("blueprints");
    const relativePath = blueprintsIdx >= 0 ? segments.slice(blueprintsIdx + 1).join("/") : segments.join("/");
...
function addNameStatusCandidates(candidates, out, previousRevision) {
    const deletedPathsBySlug = new Map();
    const entries = [];
    ...
    for (const parsed of entries) {
        const deletedSameSlug = parsed.previousPaths.length === 0
...
function upsertTransitionCandidate(candidates, candidate) {
    ...
    candidates.set(candidate.currentPath, {
        ...
        previousRevision: existing.previousPaths.length > 0 ? existing.previousRevision : candidate.previousRevision,
        previousPaths,
    });
}
function readMergeBase(cwd, baseRef) {
    try {
...
function listChangedBlueprintCandidates(cwd, options = {}) {
    try {
        const candidates = new Map();
        const mergeBase = options.baseRef ? readMergeBase(cwd, options.baseRef) : null;
        if (mergeBase) {
            const baseOut = execFileSync("git", ["diff", "--name-status", "--find-renames=20%", `${mergeBase}..HEAD`, "--", "blueprints"], {
                cwd,
                encoding: "utf8",
                stdio: ["ignore", "pipe", "pipe"],
                timeout: 1_500,
            ...
            const headOut = execFileSync("git", [
                "diff-tree",
                "--no-commit-id",
                "--name-status",
                "-r",
        ...
        const stagedOut = execFileSync("git", ["diff", "--name-status", "--find-renames=20%", "--cached", "--", "blueprints"], {
            cwd,
            encoding: "utf8",
            stdio: ["ignore", "pipe", "pipe"],
            timeout: 1_500,
        ...
        const statusOut = execFileSync("git", ["status", "--porcelain", "--untracked-files=all", "--", "blueprints"], {
            cwd,
            encoding: "utf8",
            stdio: ["ignore", "pipe", "pipe"],
            timeout: 1_500,
...
function hasHistoricalZeroTaskWaiver(markdown) {
    const frontmatterBody = readFrontmatterBody(markdown);
    if (!frontmatterBody)
        return false;
    return (/^historical_zero_task_waiver:\s*true\s*$/m.test(frontmatterBody) &&
        /^historical_zero_task_rationale:\s*\S.+$/m.test(frontmatterBody));
}
function listTrackedBlueprintPaths(cwd) {
    try {
...
function readGitFile(cwd, revision, filePath) {
    try {
...
function candidateFallbackPaths(slug, trackedPaths) {
...
function readPreviousLifecycleStatusFromCandidate(cwd, row, candidate, trackedPaths, currentStatus) {
    const previousPaths = [
...
export async function auditBlueprintLifecycleSql(cwd = process.cwd(), options = {}) {
    ...
    const structuralViolations = options.changedOnly && changedScope
        ? structural.violations.filter((violation) => violationTouchesBlueprintScope(violation, changedScope))
    ...
    const lifecycleIssues = options.changedOnly && changedScope
        ? lifecycle.issues.filter((issue) => issue.file
            ? violationTouchesBlueprintScope({ file: issue.file, message: issue.message }, changedScope)
            : true)
        : lifecycle.issues;
    ...
    for (const issue of lifecycleIssues) {
        ...
        target.push({
            ...
            message: issue.level === "warning" ? `${STALENESS_WARNING_PREFIX} ${issue.message}` : issue.message,
        });
    }
    ...
    try {
        const allBlueprints = db
            ...
            .all();
        const isAffectedSlug = (slug) => !options.changedOnly || changedScope === null || changedScope.slugs.has(slug);
        const isAffectedBlueprint = (row) => isAffectedSlug(row.slug);
        ...
        const affectedInProgressCount = affectedBlueprints.filter((row) => row.status === "in-progress").length;
        ...
        const transitionCandidates = transitionCandidateMap === null
            ? []
        ...
        if (allBlueprints.length === 0) {
            ...
            for (const row of transitionCandidates) {
                ...
                const previousRaw = candidate
                    ? readPreviousLifecycleStatusFromCandidate(cwd, row, candidate, trackedBlueprintPaths, currentStatus)
                    : null;
                if (!previousRaw)
                    continue;
                ...
                if (isLegalLifecycleTransition(previousStatus, currentStatus)) {
                    if (previousStatus === "planned" &&
                        ...
                        const taskCount = db
                            .prepare(`SELECT COUNT(*) AS total FROM tasks WHERE blueprint_slug = ?`)
