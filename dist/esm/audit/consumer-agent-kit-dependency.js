...
const STALE_REUSABLE_CALLER_PATTERNS = [
    {
        pattern: /\bskip_when_ci_secret_missing\s*:/u,
        message: 'consumer reusable-workflow caller must not pass skip_when_ci_secret_missing; the shared workflow contract no longer supports that fallback',
    },
    {
        pattern: /\bDOPPLER_(?:PREVIEW_|SERVICE_)?TOKEN\b/u,
        message: 'consumer workflow must not depend on long-lived Doppler token secrets; use OIDC/shared workflow bootstrap or direct runtime secrets instead',
    },
];
...
function readStringRecord(value) {
    if (typeof value !== 'object' || value === null || Array.isArray(value))
        return {};
    return Object.fromEntries(Object.entries(value).flatMap(([key, entryValue]) => typeof entryValue === 'string' ? [[key, entryValue]] : []));
}
function hasRecordKey(value, key) {
    if (typeof value !== 'object' || value === null || Array.isArray(value))
        return false;
    return Object.prototype.hasOwnProperty.call(value, key);
}
function readWorkspaceYaml(rootDirectory) {
...
export function auditConsumerAgentKitDependency(rootDirectory = process.cwd()) {
    ...
    if (typeof dependencies[PACKAGE_NAME] === 'string') {
        violations.push({
            ...
            message: `consumer package must not depend on ${PACKAGE_NAME} in dependencies; ` +
    ...
    if (typeof devDependencies[PACKAGE_NAME] === 'string') {
        violations.push({
            file: PACKAGE_JSON_PATH,
            message: `consumer package must not depend on ${PACKAGE_NAME} in devDependencies; ` +
                'use the global wp CLI / MCP surfaces instead',
        });
    }
    ...
    if (typeof workspaceCatalog[PACKAGE_NAME] === 'string') {
        violations.push({
            ...
            message: `consumer repo must not pin ${PACKAGE_NAME} in pnpm-workspace catalog; ` +
    ...
    if (hasRecordKey(workspaceYaml?.allowBuilds, PACKAGE_NAME)) {
        violations.push({
            file: PNPM_WORKSPACE_PATH,
            message: `consumer repo must not keep ${PACKAGE_NAME} in allowBuilds; ` +
    ...
    if (existsSync(join(rootDirectory, LOCAL_SETUP_ACTION_PATH))) {
        violations.push({
            ...
            message: 'consumer repo must not keep the local setup-webpresso GitHub action; use shared wp/global setup ownership instead',
        });
    }
    if (existsSync(join(rootDirectory, LOCAL_SETUP_MONOREPO_ACTION_PATH))) {
        violations.push({
            file: LOCAL_SETUP_MONOREPO_ACTION_PATH,
            message: 'consumer repo must not keep the local setup-monorepo GitHub action; use shared wp/global setup ownership instead',
        });
    }
    if (existsSync(join(rootDirectory, LOCAL_VERSION_HELPER_PATH))) {
        violations.push({
            file: LOCAL_VERSION_HELPER_PATH,
            message: 'consumer repo must not keep resolve-webpresso-cli-versions.js; local setup-webpresso ownership is retired',
        });
    }
...
//# sourceMappingURL=consumer-agent-kit-dependency.js.map