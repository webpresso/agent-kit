...
function detectInlineYaml(content, rel) {
...
function auditFile(filePath, maxMocks) {
    const violations = [];
    ...
    if (isUnitTestFile(filePath) && internalMocks.length > 0) {
        addMockViolation(violations, rel, 'ERROR', 'service-mock-in-unit-test', serviceBehavior, 'Rename to .integration.test.ts or use real dependencies.');
        addMockViolation(violations, rel, 'WARNING', 'service-data-mock-in-unit-test', serviceData, 'Consider importing real constants.');
