...
const p = (parts) => new RegExp(parts.join(""), "u");
const BANNED_PATTERNS = [
    {
        pattern: p([String.raw `\b`, "doppler", " run", String.raw `\b`]),
        message: "use `wp secrets run --sink <sink> --profile <profile> -- <cmd>` instead of direct doppler invocation",
    },
    {
        pattern: p([String.raw `\b`, "infisical", " run", String.raw `\b`]),
        message: "use `wp secrets run --sink <sink> --profile <profile> -- <cmd>` instead of direct infisical invocation",
    },
    {
    ...
    },
    {
        ...
        message: "use `wp secrets run --sink <sink> --profile <profile> -- <cmd>` instead of provider flags",
    },
    {
    ...
    },
    {
        ...
        message: "use `wp secrets run --sink <sink> --profile <profile> -- <cmd>` instead of the legacy with-secrets wrapper",
    },
    {
...
const LEGACY_CI_TOKEN_PATTERNS = [
    {
...
function fileNeedsSecretWorkflowAudit(content) {
    return (content.includes("with-secrets") ||
...
function fileNeedsOidc(content) {
    return (content.includes("doppler_identity_id") ||
        content.includes("infisical_identity_id") ||
        content.includes("auth-method: oidc") ||
        content.includes('method: "oidc"') ||
        content.includes("method: 'oidc'"));
}
function scanFile(fullPath, relPath, violations) {
    ...
    for (const { pattern, message } of BANNED_PATTERNS) {
        if (ALLOWED_PROVIDER_BOOTSTRAP_PATHS.has(relPath) &&
    ...
    if (!fileNeedsSecretWorkflowAudit(content) && !fileNeedsOidc(content))
        return;
...
export function auditSecretProviderQuarantine(rootDirectory = process.cwd()) {
...
//# sourceMappingURL=secret-provider-quarantine.js.map