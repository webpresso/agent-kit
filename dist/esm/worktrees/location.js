...
export function resolveManagedWorktreeRoot(homeDir = homedir()) {
    return join(homeDir, MANAGED_WORKTREE_ROOT_RELATIVE);
}
export function deriveRepoNamespace(input) {
 ...
 */
export function resolveWorktreeRoot(repoRoot, options = {}) {
    return join(resolveManagedWorktreeRoot(options.homeDir), "repos", deriveRepoNamespace({ repoRoot, originUrl: options.originUrl }));
}
export function resolveBlueprintWorktreeRoot(repoRoot, slug, options = {}) {
    return join(resolveWorktreeRoot(repoRoot, options), "blueprints", slug);
}
export function resolveOwnerWorktreePath(repoRoot, slug, options = {}) {
    return join(resolveBlueprintWorktreeRoot(repoRoot, slug, options), "owner");
}
export function resolveScratchWorktreePath(repoRoot, slug, lane, id, options = {}) {
    return join(resolveBlueprintWorktreeRoot(repoRoot, slug, options), ".scratch", `${sanitizeNamespaceSegment(lane)}-${sanitizeNamespaceSegment(id)}`);
}
/**
