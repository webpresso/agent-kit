...
import { normalizeTestSuiteName, resolveTestSuiteRuns } from "./suite.js";
...
function buildWorkspaceSuiteCommand(options) {
...
function buildWorkspaceFileSuiteCommand(files, options) {
    ...
    if (commands.length === 0) {
        throw new Error(`No explicit file targets matched suite "${options.suite}". Refusing to expand ${testFiles.length} file target${testFiles.length === 1 ? "" : "s"} into a broader suite run.`);
    }
    return commands.length === 1 ? commands[0] : { sequence: commands };
}
function buildPackageSuiteCommand(packageTargets, options) {
    ...
    const commands = packageTargets.flatMap((packageTarget) => suiteRuns.map((run) => ({
        command: resolution.command,
        args: [
            ...resolution.args,
            "exec",
...
function assertSuiteCompatible(target, options) {
    if (options.mutation) {
...
function resolveConcretePackageTarget(cwd, packageTarget) {
...
function readPackageName(cwd) {
    try {
        const parsed = JSON.parse(readPackageJsonText(cwd));
...
function explicitSuitesFor(suite) {
    return suite === "all" ? ["unit", "integration"] : [suite];
}
function createExplicitFileSuiteVitestArgs(suite, configFile) {
    const args = ["run"];
    if (configFile) {
        args.push("--config", configFile);
    }
...
function shouldBypassRecursiveWpTask(cwd, options) {
