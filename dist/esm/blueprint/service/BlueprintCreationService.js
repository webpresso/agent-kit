...
function validateGeneratedDraft(markdown, slug) {
    let blueprint;
    try {
...
export class BlueprintCreationService {
    ...
    async compileDraft(input) {
        ...
        const outputPath = path.join(this.projectRoot, 'webpresso', 'blueprints', 'draft', slug, '_overview.md');
        ...
        const markdown = replacements.reduce((currentTemplate, [placeholder, value]) => replacePlaceholder(currentTemplate, placeholder, value), template);
