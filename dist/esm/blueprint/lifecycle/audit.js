...
export function countDistinctApprovals(approvals) {
    return new Set(normalizeApprovalEntries(approvals)
        .filter((entry) => entry.verdict === "approve")
        .map((entry) => entry.reviewer)).size;
}
function normalizeApprovalEntries(approvals) {
    ...
    return entries
        ...
        verdict: String(e.verdict ?? "")
            .trim()
            .toLowerCase(),
        commit: typeof e.commit === "string" && e.commit.trim().length > 0 ? e.commit.trim() : undefined,
        evidence: typeof e.evidence === "string" && e.evidence.trim().length > 0
            ? e.evidence.trim()
            : undefined,
        rev: typeof e.rev === "string" && e.rev.trim().length > 0 ? e.rev.trim() : undefined,
        targetHash: typeof e.targetHash === "string" && e.targetHash.trim().length > 0
            ? e.targetHash.trim()
            : undefined,
    }))
        .filter((entry) => entry.reviewer.length > 0);
}
function resolveApprovalEvidencePath(file, evidence) {
    if (!evidence)
        return null;
    ...
    if (relativeToBlueprint === ".." || relativeToBlueprint.startsWith("../"))
        return null;
    if (!existsSync(resolved))
        return null;
    if (!isGitTracked(findGitRoot(file), resolved))
        return null;
    return resolved;
}
const REVIEW_ENTRY_MARKER = "<!-- wp:review-entry ";
function parseApprovalReviewRecordsFromLedger(markdown) {
    const records = [];
    for (const line of markdown.split("\n")) {
        ...
        if (trimmed.startsWith(REVIEW_ENTRY_MARKER) && trimmed.endsWith("-->")) {
            ...
            try {
                ...
                if (!isStructuredApprovalReviewRecord(parsed))
                    continue;
                records.push({
                    ...
                    verdict: String(parsed.verdict ?? "")
                        .trim()
                        .toLowerCase(),
                    artifact: typeof parsed.artifact === "string" && parsed.artifact.trim().length > 0
                        ? parsed.artifact.trim()
                        : undefined,
                    commit: typeof parsed.commit === "string" && parsed.commit.trim().length > 0
                        ? parsed.commit.trim()
                        : undefined,
                    rev: typeof parsed.rev === "string" && parsed.rev.trim().length > 0
                        ? parsed.rev.trim()
                        : undefined,
                    source: typeof parsed.source === "string" && parsed.source.trim().length > 0
                        ? parsed.source.trim()
                        : undefined,
                    targetHash: typeof parsed.targetHash === "string" && parsed.targetHash.trim().length > 0
...
function isStructuredApprovalReviewRecord(value) {
...
function resolveReviewArtifactPath(file, ledgerPath, artifact) {
    if (!artifact)
        return null;
    if (artifact === "reviews.md")
        return null;
    const resolved = resolveApprovalEvidencePath(file, artifact);
    if (!resolved)
        return null;
    if (path.resolve(resolved) === path.resolve(ledgerPath))
        return null;
    return resolved;
}
function approvalMatchesRecord(entry, record) {
    if (record.reviewer !== entry.reviewer || record.verdict !== "approve")
        return false;
    if (entry.rev !== undefined && record.rev !== entry.rev)
        return false;
    if (entry.commit !== undefined && record.commit !== entry.commit)
        return false;
    if (entry.targetHash !== undefined && record.targetHash !== entry.targetHash)
        return false;
    return true;
}
function approvalMatchesProvenanceBackedRecord(file, ledgerPath, entry, record) {
    if (!approvalMatchesRecord(entry, record))
        return false;
...
function isGitTracked(repoRoot, file) {
    if (!repoRoot)
        return false;
...
export function countDistinctLogBackedApprovals(file, approvals) {
...
export function countDistinctProvenanceBackedApprovals(file, approvals) {
    ...
    for (const entry of normalizeApprovalEntries(approvals)) {
        ...
        if (!evidencePath)
            continue;
        const records = parseApprovalReviewRecordsFromLedger(readFileSync(evidencePath, "utf8"));
        if (records.some((record) => approvalMatchesProvenanceBackedRecord(file, evidencePath, entry, record))) {
            distinct.add(entry.reviewer);
        }
    }
    return distinct.size;
}
export function validateApprovalGate(file, frontmatter) {
    if (frontmatter.type !== "blueprint")
        return [];
    const status = typeof frontmatter.status === "string" ? frontmatter.status : "";
    if (!APPROVAL_GATED_STATUSES.has(status))
        return [];
...
function normalizeInlineTaskStatus(rawStatus) {
    if (!rawStatus)
        return undefined;
    return (rawStatus
        .trim()
...
function validateOwnerBindingTruth(file, frontmatter) {
    if (frontmatter.type !== "blueprint" || frontmatter.status !== "in-progress")
        return [];
    const issues = [];
    const ownerId = typeof frontmatter.worktreeOwnerId === "string" ? frontmatter.worktreeOwnerId.trim() : "";
    const ownerBranch = typeof frontmatter.worktreeOwnerBranch === "string"
    ...
    if (!ownerId) {
        issues.push({
            file,
            level: "error",
            message: "In-progress executable blueprint is missing worktree_owner_id; start or repair it with `wp blueprint start` / `wp worktree rebind`.",
        });
    }
    if (!ownerBranch) {
        issues.push({
            file,
            level: "error",
            message: "In-progress executable blueprint is missing worktree_owner_branch; start or repair it with `wp blueprint start` / `wp worktree rebind`.",
        });
    }
