...
export function validateBlueprintTrust(input) {
    ...
    if (parsed.dossier) {
        ...
        violations.push(...validateTrustAmbiguity(parsed.dossier, input.scanTaskAmbiguity === true ? input.markdown : undefined));
        if (!parsed.dossier.readiness.promotionReady)
            violations.push({ section: "Readiness Verdict", message: "promotion-ready must be true" });
        if (parsed.dossier.readiness.unresolvedCount !== 0 ||
