...
const GENERIC_PROJECT_MARKERS = ['.agent-kitrc.json', 'pnpm-workspace.yaml', 'package.json'];
export function hasWebpressoProjectMarker(projectPath) {
    return existsSync(path.join(projectPath, WEBPRESSO_CONFIG_PATH));
}
export function hasGenericProjectMarker(projectPath) {
...
export function resolveBlueprintRoot(projectPath) {
    if (projectPath === undefined) {
        ...
        if (existsSync(path.resolve(DEFAULT_BLUEPRINTS_DIR)))
            return DEFAULT_BLUEPRINTS_DIR;
        if (existsSync(path.resolve(WEBPRESSO_BLUEPRINTS_DIR)))
