...
export const REPO_ROOT_MARKERS = ['pnpm-workspace.yaml', '.git'];
/**
...
export function findRepoRoot(startDir = process.cwd()) {
    ...
    for (;;) {
        ...
        if (parent === current) {
            ...
            throw new Error(`Could not find repo root (looked for ${REPO_ROOT_MARKERS.join(', ')} or a package.json). Started from: ${startDir}`);
        }
        current = parent;
    }
}
