...
committed `.webpresso/secrets.config.json` metadata and `@webpresso/agent-config`
for config presets. Consumer repos should not depend on `@webpresso/agent-kit`
as a project package just to get the `wp` runtime.
