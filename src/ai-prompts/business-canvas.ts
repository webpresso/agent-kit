...
export function parseBusinessCanvas(response: string): ParsedSteveResponse {
  ...
  let canvas: BusinessCanvas | undefined
  if (canvasMatch?.[1]) {
    try {
      canvas = JSON.parse(canvasMatch[1].trim()) as BusinessCanvas
    } catch {
      console.warn('Failed to parse business canvas JSON')
    }
  }

  let viability: ViabilityDecision | undefined
  if (viabilityMatch?.[1]) {
...
function isValidTargetAudience(audience: unknown): audience is TargetAudience {
...
function isValidSuccessMetric(metric: unknown): metric is SuccessMetric {
...
function hasValidCanvasStrings(c: Record<string, unknown>): boolean {
...
function hasValidCanvasArrays(c: Record<string, unknown>): boolean {
...
function hasValidCanvasAudience(c: Record<string, unknown>): boolean {
  const targetAudience = c.targetAudience as unknown[]
...
function hasValidCanvasMetrics(c: Record<string, unknown>): boolean {
  const successMetrics = c.successMetrics as unknown[]
...
function hasValidCanvasFeatures(c: Record<string, unknown>): boolean {
...
export function isValidBusinessCanvas(canvas: unknown): canvas is BusinessCanvas {
...
function isValidFeatureProposal(feature: unknown): feature is FeatureProposal {
  ...
  if (!['must-have', 'should-have', 'nice-to-have'].includes(f.priority as string)) {
    return false
  }
