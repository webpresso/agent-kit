...
export const TASK_ANALYSIS_PROMPTS: Record<AgentPersona, string> = {
...
function formatRelatedTasks(relatedTasks?: Array<{ title: string; status: string }>): string {
...
function buildReplacements(task: TaskContext): Record<string, string> {
  return {
...
export function buildTaskAnalysisPrompt(agentId: string, task: TaskContext): string {
  const normalizedId = agentId.toLowerCase() as AgentPersona
...
export function parseTaskAnalysisResponse(response: string): TaskAnalysisResponse | null {
  const jsonMatch = response.match(/```(?:json)?\s*([\s\S]*?)```/)
  const jsonStr = jsonMatch?.[1]?.trim() ?? response.trim()

  try {
    const parsed = JSON.parse(jsonStr) as TaskAnalysisResponse
...
export const AGENT_FOCUS_AREAS: Record<AgentPersona, string[]> = {
...
export const AGENT_SUGGESTED_FIELDS: Record<AgentPersona, (keyof SuggestedFields)[]> = {
