...
export const DEFAULT_DEBATE_CONFIG: Required<Omit<DebateConfig, 'type' | 'topic' | 'context'>> = {
  maxRounds: 3,
...
export const DEBATE_PROMPTS = {
  initialPrompt: (config: DebateConfig, persona: AgentPersona): string => {
    const roleContext: Record<AgentPersona, string> = {
      steve:
        'As the Business Strategist, evaluate this from a business viability, revenue impact, and strategic alignment perspective.',
      rachel:
        'As the Product Visionary, evaluate this from a user experience, accessibility, and product-market fit perspective.',
      ozby: 'As the Engineering Lead, evaluate this from a technical feasibility, code quality, and scalability perspective.',
      volker:
        'As the Clean Code Evangelist, evaluate this from a code quality, testability, and maintainability perspective.',
      jeramy:
        'As the Backend & Cloud Architect, evaluate this from a scalability, data flow, and infrastructure cost perspective.',
      rodrigo:
        'As the Sustainability & Supply Chain CTO, evaluate this from a supply chain complexity, enterprise scalability, and sustainability perspective.',
    }
  ...
  moderatorPrompt: (config: DebateConfig, state: DebateState): string => {
...
export function parsePosition(
  text: string,
): 'approve' | 'reject' | 'abstain' | 'needs_clarification' {
...
const CONFIDENCE_KEYWORDS: ReadonlyArray<{ keywords: string[]; value: number }> = [
...
function parseWordBasedConfidence(text: string): number | null {
...
export function calculateConsensus(contributions: AgentContribution[]): {
  ...
  const majorityPosition = (Object.entries(voteCounts).find(
    ([_, count]) => count === maxVotes,
  )?.[0] ?? 'abstain') as 'approve' | 'reject' | 'abstain' | 'needs_clarification'
...
export function createDebateState(config: DebateConfig): DebateState {
  return {
    id: generateDebateId(),
    config: {
...
export function addContribution(state: DebateState, contribution: AgentContribution): DebateState {
...
export function startNewRound(state: DebateState, topic?: string): DebateState {
...
export function finalizeDebate(state: DebateState, outcome: DebateOutcome): DebateState {
  return {
    ...state,
...
export function shouldContinueDebate(state: DebateState): boolean {
