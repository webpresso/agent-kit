...
describe('bundled tsconfig JSON files', () => {
  ...
  it.each(configFiles)('%s stays byte-for-byte aligned with the repo-root preset copy', async (fileName) => {
    const repositoryRoot = process.cwd()
    const sourceTarget = await readFile(join(repositoryRoot, 'src', 'config', 'tsconfig', fileName), 'utf8')
  ...
  it('publishes tsconfig preset inheritance through canonical package subpaths only', async () => {
    ...
    const reactLibrary = JSON.parse(
      await readFile(join(repositoryRoot, 'src', 'config', 'tsconfig', 'react-library.json'), 'utf8'),
    ) as { extends?: string }
    ...
    const reactRouter = JSON.parse(
      await readFile(join(repositoryRoot, 'src', 'config', 'tsconfig', 'react-router.json'), 'utf8'),
    ) as { extends?: string }
