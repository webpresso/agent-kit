...
function writeBlueprintReadme(
  ...
  writeFileSync(
    path.join(projectRoot, 'blueprints', 'README.md'),
    [
      '# Blueprints',
      '',
      ...
      '| ----- | ----: | ----------- |',
      `| \`draft/\` | ${all.draft} | early-stage sketches. Expect churn; move to \`planned/\` once scoped. |`,
      `| \`planned/\` | ${all.planned} | committed-to specs, ready to pick up. |`,
      `| \`in-progress/\` | ${all['in-progress']} | actively being executed. At most 3 active blueprints per lane. |`,
      `| \`completed/\` | ${all.completed} | execution finished and verified. Kept for reference. |`,
      `| \`parked/\` | ${all.parked} | intentionally paused. Include a reason in the spec's frontmatter. |`,
      `| \`archived/\` | ${all.archived} | superseded or abandoned. Not deleted — the record matters. |`,
      '<!-- END: blueprint-index -->',
      '',
      '## Authoring',
      '',
...
describe('applyBlueprintLifecycleToFile', () => {
  ...
  it('moves flat markdown consumer blueprints within top-level blueprints/', async () => {
    ...
    writeBlueprintReadme(projectRoot, { planned: 1 })
    await attachPassingVerification(filePath, projectRoot)
    ...
    const readme = path.join(projectRoot, 'blueprints', 'README.md')
    expect(existsSync(readme)).toBe(true)
    expect(readFileSync(readme, 'utf8')).toContain('| `planned/` | 0 |')
    expect(readFileSync(readme, 'utf8')).toContain('| `in-progress/` | 1 |')
  })
