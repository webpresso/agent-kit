...
export function validateTrustEvidence(
  repoRoot: string,
  ...
  for (const claim of dossier.claims) {
    ...
    for (const token of tokens) {
      if (token.startsWith("repo:")) {
        const rel = unescapeMarkdownPathToken(token.slice("repo:".length).trim());
...
function unescapeMarkdownPathToken(value: string): string {
...
function hasCycle(
  ...
  visiting.add(id);
  for (const next of graph.get(id) ?? []) if (hasCycle(next, graph, visiting, visited)) return true;
  visiting.delete(id);
  visited.add(id);
  return false;
}
