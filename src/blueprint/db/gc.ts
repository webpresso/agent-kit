...
function removeProjection(dbPath: string): number {
  let pruned = 0
...
function shouldPruneRepoScopedProjection(dbPath: string, now: number, ttlMs: number): boolean {
  const metadata = readProjectionMetadata(dbPath)
  if (!metadata) return false
  if (metadata.worktree_path && !existsSync(metadata.worktree_path)) return true
  return now - metadata.ingested_at > ttlMs
}

export function pruneProjectionArtifacts(
  options: PruneProjectionArtifactsOptions = {},
): PruneProjectionArtifactsResult {
  ...
  for (const repoRoot of safeListDirs(stateRoot)) {
    ...
    for (const worktreeDir of safeListDirs(legacyWorktreeRoot)) {
      ...
      if (legacyDb === preserveDbPath) continue
      if (!legacyDb.includes(LEGACY_WORKTREE_SEGMENT)) continue
