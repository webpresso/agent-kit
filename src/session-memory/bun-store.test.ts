...
describe('BunSqliteStore — insert and basic search', () => {
  ...
  it('falls back to Levenshtein when both porter and trigram find nothing', () => {
    ...
    // Result may be empty for very short content, but should not throw
...
describe('BunSqliteStore — getDb() for session operations', () => {
  it('captureEvent via getDb() + snapshot + restore round-trip', () => {
    const db = store.getDb()

    // Insert a session event directly via raw DB
    ...
    // Restore: query the snapshot back
    const snap = db
