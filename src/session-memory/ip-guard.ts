...
function embeddedIpv4Suffix(groups: readonly number[]): string {
...
function isInternalIpv6(address: string): boolean {
  ...
  const nat64WellKnown =
    first === 0x0064 && second === 0xff9b && groups.slice(2, 6).every((group) => group === 0);
  return (
    allZero ||
    address === "::1" ||
    ipv4Compatible ||
...
export async function resolveHostAddresses(
  ...
  const ipVersion = isIP(normalized);
  if (ipVersion === 4 || ipVersion === 6) return [{ address: normalized, family: ipVersion }];
