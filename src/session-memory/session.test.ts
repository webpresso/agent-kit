...
describe('SessionMemorySessionStore', () => {
  it('captures, snapshots, and restores events', () => {
    ...
    store.captureEvent({ repoHash: 'repo123456789abcd', event: { toolName: 'edit', content: 'changed session memory store' } })
    const snapshot = store.snapshot({ repoHash: 'repo123456789abcd' })
    expect(snapshot.status).toBe('complete')
    expect(snapshot.content).toContain('session memory')
    expect(store.restore({ repoHash: 'repo123456789abcd', query: 'memory' })[0]?.content).toContain('memory')
    store.close()
  })
  ...
  it('returns partial snapshots when cap is exhausted', () => {
    ...
    for (let i = 0; i < 20; i += 1) {
      store.captureEvent({ repoHash: 'repo123456789abcd', event: { toolName: 'tool', content: `event ${i}` } })
    }
  ...
  it('supports multiple handles writing with WAL enabled', () => {
    ...
    a.captureEvent({ repoHash: 'repo123456789abcd', event: { toolName: 'a', content: 'alpha write' } })
    b.captureEvent({ repoHash: 'repo123456789abcd', event: { toolName: 'b', content: 'beta write' } })
