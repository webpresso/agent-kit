 ...
 */
export function orderWorkspacePackagesForRelease<T extends WorkspaceReleasePackage>(
  packages: readonly T[],
): T[] {
  ...
  for (const pkg of packages) {
    for (const depName of pkg.workspaceDependencies) {
      if (!byName.has(depName) || depName === pkg.name) continue;
  ...
  while (ready.length > 0) {
    ...
    for (const dependentName of [...(outgoing.get(name) ?? [])].sort((a, b) =>
      a.localeCompare(b),
    )) {
      const deps = incoming.get(dependentName);
      deps?.delete(name);
      if ((deps?.size ?? 0) === 0) ready.push(dependentName);
    }
  ...
  if (ordered.length !== packages.length) {
    const remaining = packages
      ...
      .sort((a, b) => a.localeCompare(b));
    throw new Error(
      `workspace release package graph contains a cycle or unresolved local dependency: ${remaining.join(", ")}`,
    );
  }
