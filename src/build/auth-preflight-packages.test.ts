...
describe("release workflow publish path", () => {
  ...
  it("does not run local Husky hooks for automated release tag pushes", () => {
    const workflow = readWorkflow(join(repositoryRoot, ".github", "workflows", "release.yml"));
    const rootTagStep = workflow.slice(
      workflow.indexOf("- name: Create mainline release tag when missing"),
      workflow.indexOf("- name: Verify tag points at the published mainline commit"),
    );
    const workspaceReleaseStep = workflow.slice(
      workflow.indexOf("- name: Publish workspace package GitHub Releases"),
      workflow.indexOf("- name: Build native runtime binaries for the GitHub Release"),
    ...
    expect(rootTagStep).toContain('HUSKY=0 git push --force origin "refs/tags/${tag}"');
    expect(workspaceReleaseStep).toContain('HUSKY: "0"');
