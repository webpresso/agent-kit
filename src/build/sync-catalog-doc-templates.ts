 ...
 */
export function diffDocTemplateMirror(): string[] {
  ...
  for (const name of names) {
    ...
    if (
      readFileSync(join(SOURCE_DIR, name), 'utf8') !== readFileSync(join(MIRROR_DIR, name), 'utf8')
    ) {
      drift.push(name)
    }
  }
...
export function syncDocTemplateMirror(): number {
  ...
  for (const name of listFiles(MIRROR_DIR)) {
    if (!sourceFiles.includes(name)) rmSync(join(MIRROR_DIR, name))
  }
