...
const REPO_AUDIT_REGISTRY: Record<string, RepoAuditRunner> = {
  ...
  "test-isolation": async (root) =>
    (await import("#audit/repo-guardrails")).auditTestIsolation(root),
  ...
  "toolchain-isolation": async (root) =>
...
const AFFECTED_SAFE_AUDIT_KINDS = new Set<string>([
...
export function getAuditScopeSafety(kind: string): AuditScopeSafety {
  return AFFECTED_SAFE_AUDIT_KINDS.has(kind) ? "affected-safe" : "full-scan-only";
}

export function resolveGuardrailAuditKinds(root: string, scope: AuditScope = "full"): string[] {
...
function isAffectedAuditKind(kind: string): boolean {
  return (
    kind === "guardrails" || kind === "quality" || getAuditScopeSafety(kind) === "affected-safe"
  );
}
...
export function registerAuditCommand(cli: CAC): void {
  addAffectedOptions(
    cli.command("audit [kind] [target]", `Run a packaged audit (${AUDIT_KIND_LIST})`),
  )
    ...
    .action(
      async (kind: string | undefined, target: string | undefined, options: AuditActionOptions) => {
        ...
        let dispatchOptions: AuditActionOptions = options;
        let auditScope: AuditScope = "full";
        ...
        if (affected || branch) {
          ...
          const resolution = resolveAffectedTargets({
            ...
            degradedFallbackMessage: (reason) =>
              `Unable to determine affected files for audit (${reason}); falling back to full audit.`,
            degradedFailClosedMessage: (reason) =>
              `Unable to determine affected files for audit (${reason}); refusing degraded affected audit.`,
          });
