...
async function runDocsLint(target: string): Promise<number> {
  ...
  if (!blueprintFiles) {
    console.log(
      `No blueprint documents found at ${absoluteTarget} (files must declare \`doc-type: blueprint\` in frontmatter).`,
    )
    return 0
  }
