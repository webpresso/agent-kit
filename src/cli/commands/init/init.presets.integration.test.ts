/**
 * Integration tests for the OMX, OMC, and RTK scaffolder presets, exercised
 * through the full `runInit()` machinery.
 *
 * `node:child_process.spawnSync` is mocked at module-load so the presets
 * don't actually invoke `omx setup`, Claude plugin install, or RTK setup — the integration
