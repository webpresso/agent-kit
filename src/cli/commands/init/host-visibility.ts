...
import { comparePluginVersions } from "./plugin-cache-prune.js";
...
function pluginCacheSkillRoots(homeDir: string, vendor: "claude" | "codex"): string[] {
  ...
  const roots: Array<{ marketplace: string; version: string; skills: string }> = [];
  try {
    for (const marketplace of readdirSync(base)) {
      ...
      for (const version of versions) {
        ...
        if (existsSync(skills)) roots.push({ marketplace, version, skills });
      }
    }
  } catch {
  ...
  return roots
    .sort((left, right) => {
      const versionDelta = comparePluginVersions(right.version, left.version);
      if (versionDelta !== 0) return versionDelta;
      return left.marketplace.localeCompare(right.marketplace);
    })
    .map((root) => root.skills);
}
