...
export const GUARDED_NODE_MODULES_BIN_PATTERN =
  /^\[ -x (["']?)((?:\.\/|\/.*\/)?node_modules\/\.bin\/([\w-]+))\1 \] && \1\2\1 \|\| (?:true|printf .+)$/u
export const IF_GUARDED_NODE_MODULES_BIN_PATTERN = new RegExp(
  String.raw`^if \[ -x (["']?)((?:\.\/|\/.*\/)?node_modules\/\.bin\/([\w-]+))\1 \]; then \1\2\1; else ${GUARDED_FALLBACK_PATTERN}; fi$`,
  'u',
)
export const DIRECT_CLAUDE_NODE_MODULES_BIN_PATTERN =
  /^["']?\$CLAUDE_PROJECT_DIR\/node_modules\/\.bin\/([\w-]+)["']?$/u
export const GUARDED_CLAUDE_NODE_MODULES_BIN_PATTERN =
  /^\[ -x (["']?)\$CLAUDE_PROJECT_DIR\/node_modules\/\.bin\/([\w-]+)\1 \] && \1\$CLAUDE_PROJECT_DIR\/node_modules\/\.bin\/\2\1 \|\| (?:true|printf .+)$/u
export const IF_GUARDED_CLAUDE_NODE_MODULES_BIN_PATTERN = new RegExp(
  String.raw`^if \[ -x (["']?)\$CLAUDE_PROJECT_DIR\/node_modules\/\.bin\/([\w-]+)\1 \]; then \1\$CLAUDE_PROJECT_DIR\/node_modules\/\.bin\/\2\1; else ${GUARDED_FALLBACK_PATTERN}; fi$`,
  'u',
)
export const DIRECT_MANAGED_HOOK_LAUNCHER_PATTERN =
  /^(?:["']?)((?:\$CLAUDE_PROJECT_DIR\/\.claude\/hooks\/managed|(?:\.\/|\/.*\/)?\.claude\/hooks\/managed|(?:\.\/|\/.*\/)?\.codex\/managed-hooks)\/((?:wp|ak)-[\w-]+)\.sh)(?:["']?)$/u
export const GUARDED_MANAGED_HOOK_LAUNCHER_PATTERN =
  /^\[ -x (["']?)((?:\$CLAUDE_PROJECT_DIR\/\.claude\/hooks\/managed|(?:\.\/|\/.*\/)?\.claude\/hooks\/managed|(?:\.\/|\/.*\/)?\.codex\/managed-hooks)\/((?:wp|ak)-[\w-]+)\.sh)\1 \] && \1\2\1 \|\| (?:true|printf .+)$/u
export const IF_GUARDED_MANAGED_HOOK_LAUNCHER_PATTERN = new RegExp(
  String.raw`^if \[ -x (["']?)((?:\$CLAUDE_PROJECT_DIR\/\.claude\/hooks\/managed|(?:\.\/|\/.*\/)?\.claude\/hooks\/managed|(?:\.\/|\/.*\/)?\.codex\/managed-hooks)\/((?:wp|ak)-[\w-]+)\.sh)\1 \]; then \1\2\1; else ${GUARDED_FALLBACK_PATTERN}; fi$`,
  'u',
)
