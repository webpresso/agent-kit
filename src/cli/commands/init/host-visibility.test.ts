...
describe("host skill visibility", () => {
  ...
  it("prefers the newest installed plugin cache skill path over stale versions", () => {
    writeSkill(
      join(homeDir, ".claude", "plugins", "cache", "webpresso", "agent-kit", "2.3.3", "skills"),
      "claude",
    );
    writeSkill(
      join(homeDir, ".claude", "plugins", "cache", "webpresso", "agent-kit", "2.4.0", "skills"),
      "claude",
    );
    ...
    expect(audit.results[0]?.status).toBe("visible-after-restart");
    expect(audit.results[0]?.foundPaths[0]).toContain(join("agent-kit", "2.4.0", "skills"));
    expect(audit.results[0]?.foundPaths[1]).toContain(join("agent-kit", "2.3.3", "skills"));
  });
