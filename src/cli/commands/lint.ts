        ...
        const resolution = resolveAffectedTargets(
          {
            ...
            degradedFallbackMessage: (reason) =>
              `Unable to determine affected files for lint (${reason}); falling back to whole-repo lint.`,
            degradedFailClosedMessage: (reason) =>
              `Unable to determine affected files for lint --fix (${reason}); refusing a degraded whole-repo write. Rerun without --affected or pass --file explicitly.`,
          },
          deps,
        );
...
function lintEmptyMessage(scope: AffectedScope): string {
  return scope === "branch"
