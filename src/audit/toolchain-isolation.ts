...
function isExemptPackage(root: string, packagePath: string, pkg: PackageJson): boolean {
  ...
  // agent-config is the toolchain publisher itself — it legitimately ships the
  // packages that consumers are told to route through wp.
  if (pkg.name === '@webpresso/agent-config') return true
