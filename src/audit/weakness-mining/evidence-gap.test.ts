...
describe('detectEvidenceGap', () => {
  it('records a non-failing no-evidence gap for clean checkouts', () => {
    const gap = detectEvidenceGap({ records: [], candidateFiles: ['.agent/logs/pretool-guard.log'], warnings: [] })
  ...
  it('does not report a gap when records exist', () => {
    const gap = detectEvidenceGap({
      records: [{ timestamp: '2026-06-13T10:00:00.000Z', status: 'PASS', tool: 'Bash', target: 'git status', sourceFile: 'x', lineNumber: 1 }],
      candidateFiles: ['x'],
      warnings: [],
    })
