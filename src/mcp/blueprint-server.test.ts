...
function makeLocalBlueprintRepo(slug: string, content = VALID_BLUEPRINT): {
  dir: string
  overviewPath: string
} {
  const dir = mkdtempSync(path.join(tmpdir(), 'ak-bs-local-bp-'))
  mkdirSync(path.join(dir, '.agent'), { recursive: true })
  writeFileSync(path.join(dir, 'package.json'), JSON.stringify({ name: 'test' }), 'utf8')
  const overviewPath = path.join(dir, 'blueprints', 'draft', slug, '_overview.md')
...
describe('wp_blueprint_list', () => {
  ...
  it('filters by status when provided', async () => {
    const { dir } = makeLocalBlueprintRepo('list-test')
    const { registrar: lr, tools: lt } = makeRegistrar()

    try {
      await registerBlueprintTools(lr, dir)
...
describe('wp_blueprint_get', () => {
  ...
  it('returns blueprint with tasks and freshness metadata when found', async () => {
    ...
    const { dir } = makeLocalBlueprintRepo(bpSlug)
    const { registrar: lr, tools: lt } = makeRegistrar()
