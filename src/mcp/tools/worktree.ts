...
import {
  ...
  resolveWorktreePath,
  type WorktreeEntry,
} from '#cli/commands/worktree/router-dispatch'
...
const ACTIONS = ['list', 'root', 'new', 'remove', 'prune', 'refresh'] as const
const MUTATING_ACTIONS = new Set<WpWorktreeInput['action']>(['new', 'remove', 'prune', 'refresh'])
...
function toOutputEntry(entry: WorktreeEntry): z.infer<typeof worktreeOutputEntry> {
  return {
    path: entry.path,
...
function mutationGate(input: WpWorktreeInput): WpWorktreePayload | null {
  if (!MUTATING_ACTIONS.has(input.action) || input.execute === true) return null
  return {
    passed: false,
...
function discoverDefaultBaseRef(repoRoot: string): string {
  ...
  if (originHead.status === 0 && discovered.length > 0) return discovered
  return 'origin/main'
}
...
function handleNew(input: WpWorktreeInput, repoRoot: string): WpWorktreePayload {
  const entries = listEntries(repoRoot)
  let target: ReturnType<typeof resolveNewWorktreeTarget>
  try {
  ...
  const addResult = spawnSync(
    'git',
    ['worktree', 'add', '-b', target.branch, target.path, baseRef],
    {
      cwd: repoRoot,
      encoding: 'utf8',
    },
...
function handleRemove(input: WpWorktreeInput, repoRoot: string): WpWorktreePayload {
  if (input.force) {
    return {
      passed: false,
...
const tool: ToolDescriptor = {
  ...
  handler: async (raw) => {
    ...
    try {
      ...
      if (input.action === 'new') return result(handleNew(input, repoRoot))
      ...
      return result({
        ...
        nextAction:
          'No cleanup was attempted automatically; inspect the repository/worktree state and retry the bounded action.',
      })
    }
  },
}
