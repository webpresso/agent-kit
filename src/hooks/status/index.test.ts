...
import {
  ...
  deriveHookConfigSourceStatus,
  formatHostSurfaceStatusLine,
  formatHookConfigSourceLine,
  formatHookConfigSourceStatusLine,
  WP_HOOK_SPECS,
  statusCommand,
} from "./index.js";
...
describe("hook config source status", () => {
  it("lists active project and system hook sources with git path or absolute path", () => {
    const repoRoot = mkdtempSync(join(tmpdir(), "wp-hooks-source-repo-"));
    const homeRoot = mkdtempSync(join(tmpdir(), "wp-hooks-source-home-"));
    try {
      mkdirSync(join(repoRoot, ".codex"), { recursive: true });
      mkdirSync(join(repoRoot, ".opencode", "plugins"), { recursive: true });
      mkdirSync(join(homeRoot, ".claude"), { recursive: true });
      mkdirSync(join(homeRoot, ".config", "opencode", "plugins"), { recursive: true });

      writeFileSync(
        join(repoRoot, ".codex", "hooks.json"),
        JSON.stringify({ hooks: buildFullHooksMap() }),
      );
      writeFileSync(
        join(repoRoot, ".opencode", "plugins", "webpresso-hooks.js"),
        "// Generated by webpresso\nexport const WebpressoHooksPlugin = {}\n",
      );
      writeFileSync(
        join(homeRoot, ".claude", "settings.json"),
        JSON.stringify(buildFullHooksMap()),
      );
      writeFileSync(
        join(homeRoot, ".config", "opencode", "plugins", "webpresso-hooks.js"),
        "// Generated by webpresso\nexport const WebpressoHooksPlugin = {}\n",
      );

      const result = deriveHookConfigSourceStatus(repoRoot, homeRoot);
      expect(
        result.find((source) => source.vendor === "codex" && source.scope === "project"),
      ).toMatchObject({
        status: "active",
        gitPath: ".codex/hooks.json",
      });
      expect(
        result.find((source) => source.vendor === "claude" && source.scope === "system"),
      ).toMatchObject({
        status: "active",
        gitPath: null,
        filePath: join(homeRoot, ".claude", "settings.json"),
      });
      expect(
        result.find((source) => source.vendor === "opencode" && source.scope === "project"),
      ).toMatchObject({
        status: "active",
        gitPath: ".opencode/plugins/webpresso-hooks.js",
        note: "degraded plugin bridge",
      });
      expect(
        result.find((source) => source.vendor === "opencode" && source.scope === "system"),
      ).toMatchObject({
        status: "active",
        filePath: join(homeRoot, ".config", "opencode", "plugins", "webpresso-hooks.js"),
      });
    } finally {
      rmSync(repoRoot, { recursive: true, force: true });
  ...
  it("does not mark arbitrary OpenCode plugin files as active webpresso hooks", () => {
    const repoRoot = mkdtempSync(join(tmpdir(), "wp-hooks-source-foreign-"));
    try {
      mkdirSync(join(repoRoot, ".opencode", "plugins"), { recursive: true });
      writeFileSync(join(repoRoot, ".opencode", "plugins", "webpresso-hooks.js"), "export {}\n");

      const source = deriveHookConfigSourceStatus(repoRoot, repoRoot).find(
        (entry) => entry.vendor === "opencode" && entry.scope === "project",
      );
  ...
  it("formats hook source status with source scope, bounded path, and available hooks", () => {
    const line = formatHookConfigSourceStatusLine({
      vendor: "opencode",
      scope: "system",
      status: "active",
      filePath: `/Users/me/${"very-long/".repeat(40)}webpresso-hooks.js`,
      gitPath: null,
      hooks: ["plugin:session.created", "plugin:tool.execute.before"],
      note: "degraded plugin bridge",
    });

    expect(line).toContain("opencode: scope=system status=active");
    expect(line).toContain("path=/Users/me/");
    expect(line).toContain("hooks=plugin:session.created,plugin:tool.execute.before");
    expect(line).toContain("note=degraded plugin bridge");
    expect(line.length).toBeLessThan(280);
  });

  it("prints the hook sources section from statusCommand", async () => {
    const repoRoot = mkdtempSync(join(tmpdir(), "wp-hooks-status-sources-"));
    mkdirSync(join(repoRoot, ".webpresso"), { recursive: true });
    mkdirSync(join(repoRoot, ".codex"), { recursive: true });
    writeFileSync(
      join(repoRoot, ".codex", "hooks.json"),
    ...
    expect(output).toContain("[hook sources]");
    expect(output).toContain("codex: scope=project status=active gitPath=.codex/hooks.json");
    expect(output).toContain("opencode: scope=system");
  });
});
